...
public class NoteTable {
	...
	public void createTable() {
		...
		if (!query
				.exec("Create table Note (guid varchar primary key, "
		...
		if (!query
				.exec("CREATE INDEX unindexed_notess on note (indexneeded desc, guid);"))
			logger.log(logger.HIGH,
					"Note unindexed_notes index creation FAILED!!!");
		if (!query
				.exec("CREATE INDEX unsynchronized_notes on note (isDirty desc, guid);"))
			logger.log(logger.HIGH,
					"note unsynchronized_notes index creation FAILED!!!");
	...
	public void addNote(Note n, boolean isDirty) {
		...
		if (isDirty) {
			...
			query.bindValue(":content",
					enml.fixEnXMLCrap(enml.fixEnMediaCrap(n.getContent())));
		} else
		...
		if (n.getAttributes() != null) {
			created = new StringBuilder(simple.format(n.getAttributes()
					.getSubjectDate()));
			query.bindValue(":attributeSubjectDate", created.toString());
			query.bindValue(":attributeLatitude", n.getAttributes()
					.getLatitude());
			query.bindValue(":attributeLongitude", n.getAttributes()
					.getLongitude());
			query.bindValue(":attributeAltitude", n.getAttributes()
					.getAltitude());
			query.bindValue(":attributeAuthor", n.getAttributes().getAuthor());
			query.bindValue(":attributeSource", n.getAttributes().getSource());
			query.bindValue(":attributeSourceUrl", n.getAttributes()
					.getSourceURL());
			query.bindValue(":attributeSourceApplication", n.getAttributes()
		...
		if (getQueryWithContent == null) {
			...
			if (!getQueryWithContent
				...
				logger.log(logger.EXTREME,
						"Note SQL select prepare with content has failed.");
		...
		if (getQueryWithoutContent == null) {
			...
			if (!getQueryWithoutContent
					.prepare("Select "
							...
							+ " from Note where guid=:guid and isExpunged=false")) {
				logger.log(logger.EXTREME,
						"Note SQL select prepare without content has failed.");
	...
	// Get a note by Guid
	public Note getNote(String noteGuid, boolean loadContent,
			boolean loadResources, boolean loadRecognition, boolean loadBinary,
			boolean loadTags) {
		...
		Note n = mapNoteFromQuery(query, loadContent, loadResources,
				loadRecognition, loadBinary, loadTags);
	...
	// Get a note by Guid
	public Note mapNoteFromQuery(NSqlQuery query, boolean loadContent,
			boolean loadResources, boolean loadRecognition, boolean loadBinary,
			boolean loadTags) {
		...
		if (loadContent) {
			...
			String unicode = codec.fromUnicode(query.valueString(17))
					.toString();

			// This is a hack. Basically I need to convert HTML Entities to
			// "normal" text, but if I
			// convert the &lt; character to < it will mess up the XML parsing.
			// So, to get around this
			// I am "bit stuffing" the &lt; to &&lt; so StringEscapeUtils
			// doesn't unescape it. After
			...
			if (Global.enableHTMLEntitiesFix && unicode.indexOf("&#") > 0) {
				...
				// unicode =
				// codec.fromUnicode(StringEscapeUtils.unescapeHtml(unicode)).toString();
				// unicode = unicode.replace("&_lt;", "&lt;");
				// System.out.println("************************");
				int j = 1;
				for (int i = buffer.indexOf("&#"); i != -1
						&& buffer.indexOf("&#", i) > 0; i = buffer.indexOf(
						"&#", i + 1)) {
		...
		if (loadResources)
			n.setResources(noteResourceTable.getNoteResources(n.getGuid(),
					loadBinary));
		if (loadRecognition) {
			if (n.getResources() == null) {
				List<Resource> resources = noteResourceTable
						.getNoteResourcesRecognition(n.getGuid());
				n.setResources(resources);
			} else {
				// We need to merge the recognition resources with the note
				// resources retrieved earlier
				for (int i = 0; i < n.getResources().size(); i++) {
					Resource r = noteResourceTable.getNoteResourceRecognition(n
							.getResources().get(i).getGuid());
	...
	public void updateNoteTitle(String guid, String title) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Update Note set title=:title, isDirty=true where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note title sql prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	...
	public void updateNoteCreatedDate(String guid, QDateTime date) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Update Note set created=:created, isDirty=true where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note creation update sql prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	...
	public void updateNoteAlteredDate(String guid, QDateTime date) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Update Note set updated=:altered, isDirty=true where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note altered sql prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	...
	public void updateNoteSubjectDate(String guid, QDateTime date) {
		...
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note subject date sql prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
		...
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note subject date date has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	...
	public void updateNoteAuthor(String guid, String author) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Update Note set attributeAuthor=:author, isDirty=true where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note author sql prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	...
	public void updateNoteGeoTags(String guid, Double lon, Double lat,
			Double alt) {
		logger.log(logger.HIGH, "Entering NoteTable.updateNoteGeoTags");
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Update Note set attributeLongitude=:longitude, "
	...
	public void updateNoteSourceUrl(String guid, String url) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Update Note set attributeSourceUrl=:url, isDirty=true where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note url sql prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	...
	public void updateNoteNotebook(String guid, String notebookGuid,
		...
		// conflicting change we move it to the local notebook without deleting
		// it
		...
		if (expungeFromRemote) {
			if (!notebookTable.isNotebookLocal(currentNotebookGuid)
					& notebookTable.isNotebookLocal(notebookGuid)) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Update Note set notebookGuid=:notebook, isDirty=true where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note notebook sql prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	...
	public void updateNoteContent(String guid, String content) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Update Note set content=:content, updated=CURRENT_TIMESTAMP(), isDirty=true, indexNeeded=true, "
						+ " thumbnailneeded=true where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note content sql prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	...
	public void expungeNote(String guid, boolean permanentExpunge,
			boolean needsSync) {
	...
	public void expungeNotesByNotebook(String notebookGuid,
			boolean permanentExpunge, boolean needsSync) {
	...
	public void updateNoteSequence(String guid, int sequence) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Update Note set updateSequenceNumber=:sequence where guid=:guid");
	...
	public void updateNoteGuid(String oldGuid, String newGuid) {
		...
		if (!check) {
			logger.log(logger.MEDIUM,
					"Note guid update failed for noteresources.");
			logger.log(logger.MEDIUM, resQuery.lastError());
		}

		// ICHANGED
		histQuery
				.prepare("Update history set guid1=:newGuid where guid1=:oldGuid");
		...
		histQuery
				.prepare("Update history set guid2=:newGuid where guid2=:oldGuid");
		histQuery.bindValue(":newGuid", newGuid);
		histQuery.bindValue(":oldGuid", oldGuid);
		check = histQuery.exec();
		if (!check) {
	...
	public boolean isNoteExpunged(String guid) {
		...
		if (!query.exec())
			logger.log(logger.EXTREME,
					"note.isNoteExpunged SQL retrieve has failed.");
		boolean retVal = query.next();
		return retVal;
	}

	// This is a convience method to check if a note exists & update/create
	// based upon it
	public void syncNote(Note note) {
		...
		if (note.getAttributes() != null
				&& note.getAttributes().getSourceApplication() != null) {
	...
	public List<Note> getDirty() {
		...
		check = query
				.exec("Select guid from Note where isDirty = true and isExpunged = false and notebookGuid not in (select guid from notebook where local = true or linked = true)");
		if (!check)
			logger.log(logger.EXTREME, "Note SQL retrieve has failed: "
					+ query.lastError().toString());
		...
		logger.log(logger.LOW, "Dirty local notes: "
				+ new Integer(notes.size()).toString());
		return notes;
	}
	...
	public List<Note> getDirtyLinkedNotes() {
		...
		check = query
				.exec("Select guid from Note where isDirty = true and isExpunged = false and notebookGuid in (select guid from notebook where linked = true)");
		if (!check)
			logger.log(logger.EXTREME, "Note SQL retrieve has failed: "
					+ query.lastError().toString());
	...
	public List<Note> getDirtyLinked(String notebookGuid) {
		...
		if (!check)
			logger.log(logger.EXTREME,
					"Note SQL retrieve has failed getting dirty linked notes: "
							+ query.lastError().toString());
		...
		logger.log(logger.LOW, "Dirty local notes for notebook " + notebookGuid
				+ ": " + new Integer(notes.size()).toString());
		return notes;
	}
	...
	public List<String> getNotesByNotebook(String notebookGuid) {
		...
		check = query
				.prepare("Select guid from Note where notebookguid=:notebookguid");
		if (!check)
			logger.log(logger.EXTREME, "Note SQL retrieve has failed: "
					+ query.lastError().toString());
		query.bindValue(":notebookguid", notebookGuid);
	...
	public boolean isNoteDirty(String guid) {
		...
		check = query
				.prepare("Select guid from Note where isDirty = true and guid=:guid");
		query.bindValue(":guid", guid);
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME, "Note SQL retrieve has failed: "
	...
	public int getDirtyCount() {
		...
		// query.next();
		// logger.log(logger.LOW, "dirty count (active only): "
		// +query.valueString(0));
		...
		// query.next();
		// logger.log(logger.LOW, "dirty count (no l&l notebooks): "
		// +query.valueString(0));
		...
		// Note n = getNote(recs.get(i).getGuid(), true, true, true, false,
		// true);
	...
	// Reset a note sequence number to zero. This is useful for moving
	// conflicting notes
	public void resetNoteSequence(String guid) {
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Update Note set updateSequenceNumber=0, isDirty=true where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note ResetSequence sql prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
		query.bindValue(":guid", guid);
		check = query.exec();
		if (!check) {
			logger.log(logger.EXTREME,
					"Update note sequence number has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	}

	// Update a note resource by the hash
	public void updateNoteResourceGuidbyHash(String noteGuid, String resGuid,
			String hash) {
		NSqlQuery query = new NSqlQuery(db.getResourceConnection());
		/*
		 * query.prepare(
		 * "Select guid from NoteResources where noteGuid=:noteGuid and datahash=:hex"
		 * ); query.bindValue(":noteGuid", noteGuid); query.bindValue(":hex",
		 * hash); query.exec(); if (!query.next()) { logger.log(logger.LOW,
		 * "Error finding note resource in RNoteTable.updateNoteResourceGuidbyHash.  GUID="
		 * +noteGuid +" resGuid="+ resGuid+" hash="+hash); return; } String guid
		 * = query.valueString(0);
		 */
	...
	public List<String> expungeIgnoreSynchronizedNotes(List<String> notebooks,
			List<String> tags, List<String> linked) {
		...
		for (int i = 0; i < linked.size(); i++) {
			String notebookGuid = db.getLinkedNotebookTable().getNotebookGuid(
					linked.get(i));
			if (notebookGuid != null && !notebookGuid.trim().equals("")) {
				List<Tag> linkedTags = db.getTagTable().getTagsForNotebook(
						notebookGuid);
				for (int j = 0; j < linkedTags.size(); j++)
					db.getTagTable().expungeTag(linkedTags.get(j).getGuid(),
							false);
	...
	public List<Pair<String, String>> findNotesByTitle(String text) {
		...
		check = query
				.prepare("Select guid,title from Note where lower(title) like :title");
		if (!check)
			logger.log(logger.EXTREME,
					"Note SQL prepare for search by title has failed: "
							+ query.lastError().toString());
	...
	public List<String> getUnindexed() {
		...
		if (!query
				.exec("Select guid from Note where isExpunged = false and indexNeeded = true and DATEDIFF('MINUTE',updated,CURRENT_TIMESTAMP)>5"))
			logger.log(logger.EXTREME,
					"Note SQL retrieve has failed on getUnindexed().");
	...
	public List<String> getNextUnindexed(int limit) {
		...
		if (!query
				.exec("Select guid from Note where isExpunged = false and indexNeeded = true and DATEDIFF('MINUTE',Updated,CURRENT_TIMESTAMP)>5 limit "
						+ limit))
			logger.log(logger.EXTREME,
					"Note SQL retrieve has failed on getUnindexed().");
	...
	public void updateNoteMetadata(NoteMetadata meta) {
		NSqlQuery query = new NSqlQuery(db.getConnection());
		if (!query
				.prepare("Update Note set titleColor=:color, pinned=:pinned, attributeSourceApplication=:metaString where guid=:guid"))
			logger.log(logger.EXTREME,
					"Note SQL prepare has failed on updateNoteMetadata.");
		...
		if (!query.exec())
			logger.log(logger.EXTREME,
					"Note SQL exec has failed on updateNoteMetadata.");
		return;
	}
	...
	public HashMap<String, NoteMetadata> getNotesMetaInformation() {
		...
		if (!query.exec("Select guid,titleColor, isDirty, pinned from Note"))
			logger.log(logger.EXTREME,
					"Note SQL retrieve has failed on getNoteMetaInformation.");
	...
	public NoteMetadata getNoteMetaInformation(String guid) {
		...
		if (!query
				.prepare("Select guid,titleColor, isDirty, pinned from Note where guid=:guid")) {
			logger.log(logger.EXTREME,
					"Note SQL retrieve has failed on getNoteMetaInformation.");
			return null;
		}
	...
	public void setThumbnailNeeded(String guid, boolean needed) {
		...
		check = query
				.prepare("Update note set thumbnailneeded = :needed where guid=:guid");
		query.bindValue(":guid", guid);
		query.bindValue(":needed", needed);
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME, "Note SQL set thumbail needed failed: "
					+ query.lastError().toString());
	...
	public boolean isThumbnailNeeded(String guid) {
		...
		check = query
				.prepare("select thumbnailneeded from note where guid=:guid");
		query.bindValue(":guid", guid);
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME,
					"Note SQL isThumbnailNeeded query failed: "
							+ query.lastError().toString());
	...
	public void setThumbnail(String guid, QByteArray thumbnail) {
		...
		check = query
				.prepare("Update note set thumbnail = :thumbnail where guid=:guid");
		query.bindValue(":guid", guid);
		query.bindValue(":thumbnail", thumbnail.toByteArray());
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME, "Note SQL set thumbail failed: "
					+ query.lastError().toString());
	...
	public QByteArray getThumbnail(String guid) {
		...
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME, "Note SQL get thumbail failed: "
					+ query.lastError().toString());
		// Get a list of the notes
		if (query.next()) {
			try {
	...
	public HashMap<String, QPixmap> getThumbnails() {
		...
		check = query
				.prepare("Select guid,thumbnail from note where thumbnailneeded=false and isExpunged=false");
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME, "Note SQL get thumbail failed: "
					+ query.lastError().toString());
		...
		while (query.next()) {
			try {
				...
				logger.log(logger.HIGH,
						"Error retrieving thumbnail " + e.getMessage());
			}
		}
		return map;
	}
	...
	public List<String> findThumbnailsNeeded() {
		...
		check = query
				.prepare("select guid from note where thumbnailneeded=true and isExpunged=false and DATEDIFF('MINUTE',updated,CURRENT_TIMESTAMP)>5 limit 5");
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME,
					"Note SQL findThumbnailsNeeded query failed: "
							+ query.lastError().toString());
	...
	public int getThumbnailNeededCount() {
		...
		check = query
				.prepare("select count(guid) from note where thumbnailneeded=true and isExpunged=false and DATEDIFF('MINUTE',updated,CURRENT_TIMESTAMP)>5 limit 2");
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME,
					"Note SQL findThumbnailNeededCount query failed: "
							+ query.lastError().toString());
	...
	public String findAlternateGuid(String guid) {
		...
		check = query
				.prepare("select guid from note where original_guid=:guid");
		query.bindValue(":guid", guid);
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME,
					"Note SQL findAlternateguid query failed: "
							+ query.lastError().toString());
	...
	public boolean guidExists(String guid) {
		...
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME, "Note SQL guidExists query failed: "
					+ query.lastError().toString());
	...
	// outside of NN
	public void updateResourceContentHash(String guid, String oldHash,
			String newHash) {
		...
		for (; position > -1;) {
			...
			if (hash.equalsIgnoreCase(oldHash)) {
				...
				String content = n.getContent().substring(0, position)
						+ newSegment + n.getContent().substring(endPos);
				NSqlQuery query = new NSqlQuery(db.getConnection());
	...
	private String buildMetadataString(NoteMetadata meta) {
		StringBuffer value = new StringBuffer(
				removeExistingMetaString(meta.getGuid()));
		...
		if (meta.getColor() != -1) {
			metaString.append("titleColor="
					+ new Integer(meta.getColor()).toString() + ";");
		}
		if (metaString.length() > 0) {
			...
			// string.
			if (value.length() > 1
					&& (!value.toString().trim().endsWith(";") || !value
							.toString().trim().endsWith(";")))
				value.append("; ");
	...
	// This will remove the existing metadata string from the
	// attribute.sourceApplication string.
	private String removeExistingMetaString(String guid) {
		NSqlQuery query = new NSqlQuery(db.getConnection());

		if (!query
				.prepare("Select attributeSourceApplication from Note where guid=:guid")) {
			logger.log(logger.EXTREME,
					"Note SQL retrieve has failed in removeExistingMetaString.");
			return null;
		}
		query.bindValue(":guid", guid);
		query.exec();
