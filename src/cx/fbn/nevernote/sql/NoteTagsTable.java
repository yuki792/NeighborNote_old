...
public class NoteTagsTable {
	...
	void prepareGetNoteTagsQuery() {
		...
		getNoteTagsQuery
				.prepare("Select TagGuid from NoteTags where noteGuid = :guid");
	}
	...
	public void saveNoteTag(String noteGuid, String tagGuid) {
		...
		if (!check)
			logger.log(logger.EXTREME,
					"RNoteTagsTable.saveNoteTag prepare has failed.");
		query.bindValue(":guid", noteGuid);
		query.exec();
		if (!check) {
			logger.log(logger.MEDIUM,
					"RNoteTagsTable.saveNoteTag has failed to set note as dirty.");
			logger.log(logger.MEDIUM, query.lastError());
		}
	}
	...
	public void deleteNoteTag(String noteGuid) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Delete from NoteTags where noteGuid = :noteGuid");
		if (!check)
	...
	public List<Pair<String, Integer>> getTagCounts() {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		if (!query
				.exec("select tagguid, count(noteguid) from notetags group by tagguid;")) {
