...
public class SavedSearchTable {
	...
	public void createTable() {
		...
		if (!query
				.exec("Create table SavedSearch (guid varchar primary key, "
	...
	public List<SavedSearch> getAll() {
		...
		if (!check)
			logger.log(logger.EXTREME,
					"SavedSearch SQL retrieve has failed in getAll().");
		while (query.next()) {
	...
	public SavedSearch getSavedSearch(String guid) {
		...
		if (!check)
			logger.log(logger.EXTREME,
					"SavedSearch SQL prepare has failed in getSavedSearch.");
		query.bindValue(":guid", guid);
		query.exec();
		if (!check)
			logger.log(logger.EXTREME,
					"SavedSearch SQL retrieve has failed in getSavedSearch.");
		if (query.next()) {
			tempSearch = new SavedSearch();
			tempSearch.setGuid(query.valueString(0));
			tempSearch.setName(query.valueString(1));
	...
	public void updateSavedSearch(SavedSearch search, boolean isDirty) {
		...
		if (!check) {
			logger.log(logger.EXTREME,
					"SavedSearch SQL update prepare has failed.");
	...
	public void expungeSavedSearch(String guid, boolean needsSync) {
		...
		check = query.prepare("delete from SavedSearch " + "where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"SavedSearch SQL delete prepare has failed.");
			logger.log(logger.EXTREME, query.lastError().toString());
		}
		query.bindValue(":guid", guid);
		check = query.exec();
	...
	public void addSavedSearch(SavedSearch search, boolean isDirty) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Insert Into SavedSearch (guid, query, sequence, format, name, isDirty)"
	...
	public void updateSavedSearchSequence(String guid, int sequence) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Update SavedSearch set sequence=:sequence where guid=:guid");
	...
	public void updateSavedSearchGuid(String oldGuid, String newGuid) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Update SavedSearch set guid=:newGuid where guid=:oldGuid");
	...
	public List<SavedSearch> getDirty() {
		...
		if (!check)
			logger.log(logger.EXTREME,
					"SavedSearch getDirty prepare has failed.");
		while (query.next()) {
	...
	public String findSavedSearchByName(String name) {
		...
		if (!query.exec())
			logger.log(logger.EXTREME,
					"SavedSearch SQL retrieve has failed in findSavedSearchByName().");
		String val = null;
		if (query.next())
	...
	public boolean exists(String guid) {
		...
		if (!query.exec())
			logger.log(logger.EXTREME,
					"SavedSearch SQL retrieve has failed in exists().");
	...
	public void resetDirtyFlag(String guid) {
		...
		if (!query.exec())
			logger.log(logger.EXTREME,
					"Error resetting SavedSearch dirty field in resetDirtyFlag().");
	}
	...
	public QIcon getIcon(String guid) {
		...
		if (!query.prepare("Select icon from SavedSearch where guid=:guid"))
			logger.log(logger.EXTREME,
					"Error preparing saved search icon select.");
		query.bindValue(":guid", guid);
		if (!query.exec())
	...
	public void setIcon(String guid, QIcon icon, String type) {
		...
		if (icon == null) {
			if (!query
					.prepare("update SavedSearch set icon=null where guid=:guid"))
				logger.log(logger.EXTREME,
						"Error preparing saved search icon set.");
		} else {
			if (!query
					.prepare("update SavedSearch set icon=:icon where guid=:guid"))
		...
		if (!query.exec())
			logger.log(logger.LOW,
					"Error setting SavedSearch icon. " + query.lastError());
	}
