...
public class LinkedNotebookTable {
	...
	public void createTable() {
		...
		if (!query
				.exec("Create table LinkedNotebook (guid VarChar primary key, "
						+ "shareName VarChar, " + "username VarChar, "
						+ "shardID VarChar, " + "shareKey VarChar, "
						+ "uri VarChar, " + "updateSequenceNumber Long,"
						+ "lastSequenceNumber Integer,"
						+ "lastSequenceDate Long," + "notebookGuid VarChar,"
						+ "isDirty boolean)"))
	...
	public void addNotebook(LinkedNotebook tempNotebook, boolean isDirty) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Insert Into LinkedNotebook (guid, shareName, username,  "
	...
	public void expungeNotebook(String id, boolean needsSync) {
		...
		if (!check) {
			logger.log(logger.EXTREME,
					"LinkedNotebook SQL delete prepare has failed.");
	...
	public boolean exists(String id) {
		NSqlQuery query = new NSqlQuery(db.getConnection());
		boolean check = query
				.prepare("Select guid from linkednotebook where guid=:guid");
		query.bindValue(":guid", id);
		check = query.exec();
		if (!check) {
			logger.log(logger.MEDIUM,
					"LinkedNotebook Table exists check failed.");
			logger.log(logger.MEDIUM, query.lastError().toString());
		}
		if (query.next())
			return true;
	...
	public void updateNotebook(LinkedNotebook tempNotebook, boolean isDirty) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Update LinkedNotebook set guid=:guid, shareName=:shareName, "
	...
	public List<LinkedNotebook> getAll() {
		...
		check = query
				.exec("Select guid, shareName, username, shardID, shareKey, uri "
						+ " from LinkedNotebook");
		if (!check)
	...
	public LinkedNotebook getNotebook(String guid) {
		...
		check = query
				.prepare("Select guid, shareName, username, shardID, shareKey, uri "
						+ " from LinkedNotebook where guid=:guid");
		if (!check)
			logger.log(logger.EXTREME,
					"Notebook SQL retrieve notebook prepare has failed.");
	...
	public long getLastSequenceDate(String guid) {
		...
		if (!check)
			logger.log(logger.EXTREME,
					"LinkedNotebook SQL retrieve last sequence date has failed.");
		if (query.next()) {
	...
	public String getNotebookGuid(String guid) {
		...
		if (!check)
			logger.log(logger.EXTREME,
					"LinkedNotebook SQL retrieve of notebookguid by guidhas failed.");
		if (query.next()) {
	...
	public String findNotebookByShareName(String name) {
		...
		if (!query.exec())
			logger.log(logger.EXTREME,
					"Linked notebook SQL retrieve by share name has failed.");
		String val = null;
		if (query.next())
	...
	public List<String> getDirtyGuids() {
		...
		check = query
				.exec("Select guid from LinkedNotebook where isDirty = true");
		if (!check)
			logger.log(logger.EXTREME,
					"LinkedNotebook SQL retrieve has failed in getdirtyIds.");
		while (query.next()) {
