...
public class SharedNotebookTable {
	...
	public void createTable() {
		...
		if (!query.exec("Create table SharedNotebook (id long primary key, "
				+ "userid Integer, " + "notebookGuid VarChar, "
				+ "email VarChar, " + "notebookModifiable boolean, "
				+ "requireLogin boolean, " + "serviceCreated timestamp, "
				+ "shareKey VarChar,  " + "username VarChar, " + "icon blob, "
				+ "isDirty boolean)"))
	...
	public void addNotebook(SharedNotebook tempNotebook, boolean isDirty) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Insert Into SharedNotebook (id, userid, notebookGuid, email,  "
		...
		query.bindValue(":notebookModifiable",
				tempNotebook.isNotebookModifiable());
	...
	public boolean exists(long id) {
		NSqlQuery query = new NSqlQuery(db.getConnection());
		logger.log(logger.EXTREME, "Checking if shared notebook " + id
				+ " exists");
		boolean check = query
				.prepare("Select id from sharednotebook where id=:id");
		query.bindValue(":id", id);
		check = query.exec();
		if (!check) {
			logger.log(logger.MEDIUM,
					"SharedNotebook Table exists check failed.");
			logger.log(logger.MEDIUM, query.lastError().toString());
		}
		if (query.next())
			return true;
	...
	public void expungeNotebook(long id, boolean needsSync) {
		...
		check = query.prepare("delete from SharedNotebook " + "where id=:id");
		if (!check) {
			logger.log(logger.EXTREME,
					"SharedNotebook SQL delete prepare has failed.");
		...
		if (needsSync) {
			...
			deletedTable.addDeletedItem(new Long(id).toString(),
					"SharedNotebook");
		}
	}
	...
	public void expungeNotebookByGuid(String id, boolean needsSync) {
		...
		check = query.prepare("delete from SharedNotebook " + "where guid=:id");
		if (!check) {
			logger.log(logger.EXTREME,
					"SharedNotebook SQL delete by notebook guid prepare has failed.");
			logger.log(logger.EXTREME, query.lastError().toString());
		}
		query.bindValue(":id", id);
		check = query.exec();
		if (!check)
			logger.log(logger.MEDIUM,
					"SharedNotebook delete by notebook guid failed.");
	...
	public void updateNotebook(SharedNotebook tempNotebook, boolean isDirty) {
		...
		logger.log(logger.EXTREME,
				"Notebook Guid: " + tempNotebook.getNotebookGuid());
		...
		for (int i = 0; i < list.size(); i++) {
			...
			logger.log(logger.EXTREME, "Notebook Guid: "
					+ list.get(i).getNotebookGuid());
			logger.log(logger.EXTREME, "Share Key: "
					+ list.get(i).getShareKey());
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Update SharedNotebook set userid=:userid, notebookGuid=:notebook, "
	...
	public List<SharedNotebook> getAll() {
		...
		check = query
				.exec("Select id, userid, notebookGuid, email, notebookModifiable, requireLogin, "
						+ "serviceCreated, "
		...
		while (query.next()) {
			...
			try {
				tempNotebook.setServiceCreated(indfm
						.parse(query.valueString(6)).getTime());
	...
	public List<SharedNotebook> getForNotebook(String guid) {
		...
		check = query
				.prepare("Select id, userid, notebookGuid, email, notebookModifiable, requireLogin, "
						+ "serviceCreated, "
						+ "shareKey, username from SharedNotebook where notebookGuid=:notebookGuid ");
		if (!check)
			logger.log(logger.EXTREME,
					"SharedNotebook getForNotebook SQL prepare has failed.");
		query.bindValue(":notebookGuid", guid);
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME,
					"SharedNotebook getForNotebook SQL exec has failed.");
	...
	public List<Long> getDirtyIds() {
		...
		check = query
				.exec("Select id from SharedNotebook where isDirty = true");
		if (!check)
			logger.log(logger.EXTREME,
					"SharedNotebook SQL retrieve has failed in getdirtyIds.");
		while (query.next()) {
