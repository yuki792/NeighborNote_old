...
public class NoteResourceTable {
	...
	public void createTable() {
		...
		if (!query
				.exec("Create table NoteResources (guid varchar primary key, "
		...
		if (!query
				.exec("CREATE INDEX unindexed_resources on noteresources (indexneeded desc, guid);"))
			logger.log(logger.HIGH,
					"Noteresources unindexed_resources index creation FAILED!!!");
		if (!query
				.exec("CREATE INDEX resources_dataheshhex on noteresources (datahash, guid);"))
			logger.log(logger.HIGH,
					"Noteresources resources_datahash index creation FAILED!!!");
		if (!query
				.exec("create index RESOURCES_GUID_INDEX on noteresources (noteGuid, guid);"))
			logger.log(logger.HIGH,
					"Noteresources resources_datahash index creation FAILED!!!");
	}
	...
	public void resetDirtyFlag(String guid) {
		...
		if (!query.exec())
			logger.log(
					logger.EXTREME,
					"Error resetting noteresource dirty field. "
							+ query.lastError());
		else
			query.exec("commit");
	}
	...
	public void setIndexNeeded(String guid, Boolean indexNeeded) {
		...
		if (!query.exec())
			logger.log(
					logger.EXTREME,
					"Error setting noteresource indexneeded field: "
							+ query.lastError());
		else
			query.exec("commit");
	}
	...
	public List<String> getNextUnindexed(int limit) {
		...
		if (!query
				.exec("Select guid from NoteResources where indexNeeded = true limit "
						+ limit))
			logger.log(logger.EXTREME,
					"NoteResources SQL retrieve has failed on getNextUnindexed(): "
							+ query.lastError());
	...
	public List<String> getUnindexed() {
		...
		if (!query
				.exec("Select guid from NoteResources where indexNeeded = true"))
			logger.log(logger.EXTREME,
					"NoteResources SQL retrieve has failed on getUnindexed(): "
							+ query.lastError());
	...
	public List<String> getAll() {
		...
		if (!query.exec())
			logger.log(
					logger.EXTREME,
					"Error getting all note resource guids. "
							+ query.lastError());
	...
	public List<String> findInkNotes() {
		...
		if (!query.exec())
			logger.log(logger.EXTREME, "Error searching for ink notes. "
					+ query.lastError());
	...
	public void saveNoteResource(Resource r, boolean isDirty) {
		logger.log(logger.HIGH, "Entering saveNoteResources: isDirty "
				+ isDirty);
		boolean check;
		...
		check = query
				.prepare("Insert Into NoteResources ("
		...
		if (!check) {
			logger.log(logger.EXTREME,
					"NoteResource SQL insert prepare has failed.");
		...
		if (r.getData() != null) {
			query.bindValue(":dataHash", byteArrayToHexString(r.getData()
					.getBodyHash()));
		...
		if (r.getRecognition() != null) {
			query.bindValue(":recognitionHash", r.getRecognition()
					.getBodyHash());
			query.bindValue(":recognitionSize", r.getRecognition().getSize());
			if (r.getRecognition().getBody() != null)
				query.bindValue(":recognitionBody", new String(r
						.getRecognition().getBody()));
			else
		...
		if (r.getAttributes() != null) {
			query.bindValue(":attributeSourceUrl", r.getAttributes()
					.getSourceURL());
			StringBuilder ts = new StringBuilder(simple.format(r
					.getAttributes().getTimestamp()));
			query.bindValue(":attributeTimestamp", ts.toString());
			query.bindValue(":attributeLatitude", r.getAttributes()
					.getLatitude());
			query.bindValue(":attributeLongitude", r.getAttributes()
					.getLongitude());
			query.bindValue(":attributeAltitude", r.getAttributes()
					.getAltitude());
			query.bindValue(":attributeCameraMake", r.getAttributes()
					.getCameraMake());
			query.bindValue(":attributeCameraModel", r.getAttributes()
					.getCameraModel());
			query.bindValue(":attributeClientWillIndex", r.getAttributes()
					.isClientWillIndex());
			query.bindValue(":attributeRecoType", r.getAttributes()
					.getRecoType());
			query.bindValue(":attributeFileName", r.getAttributes()
					.getFileName());
			query.bindValue(":attributeAttachment", r.getAttributes()
					.isAttachment());
		}
	...
	public String getNoteResourceGuidByHashHex(String noteGuid, String hash) {
		logger.log(logger.HIGH,
				"Entering DBRunner.getNoteResourceGuidByHashHex");
		...
		if (check)
			logger.log(logger.EXTREME,
					"NoteResource SQL select prepare was successful.");
		else {
			logger.log(logger.EXTREME,
					"NoteResource SQL select prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
		}
		...
		if (!check) {
			logger.log(logger.MEDIUM,
					"dbRunner.getNoteResourceGuidByHashHex Select failed."
							+ "Note Guid:" + noteGuid + "Data Body Hash:"
							+ hash);
			logger.log(logger.MEDIUM, query.lastError());
		}
		if (!query.next()) {
	...
	// Get a note resource from the database by it's hash value
	public Resource getNoteResourceDataBodyByHashHex(String noteGuid,
			String hash) {
		logger.log(logger.HIGH,
				"Entering DBRunner.getNoteResourceDataBodyByHash");
		...
		if (!query.exec()) {
			logger.log(logger.MEDIUM, "NoteResource Select failed."
					+ "Note Guid:" + noteGuid + "Data Body Hash:" + hash);
			logger.log(logger.MEDIUM, query.lastError());
		}
		if (!query.next()) {
			logger.log(logger.MEDIUM, "Note Resource not found.");
		...
		logger.log(logger.HIGH,
				"Leaving DBRunner.getNoteResourceDataBodyByHash");
		return r;
	}
	...
	public Resource getNoteResource(String guid, boolean withBinary) {
		...
		queryString = new String(
				"Select guid, noteGuid, mime, width, height, duration, "
		...
		if (query.next()) {
			...
			r.setUpdateSequenceNum(new Integer(query.valueString(7))); // update
																		// sequence
																		// number
			...
			if (query.valueObject(10) != null)
				rec.setBodyHash(query.valueString(10).getBytes()); // Recognition
																	// Hash
			...
			a.setClientWillIndex(booleanValue(query.valueString(17).toString(),
					false)); // Camera Model
			a.setRecoType(stringValue(query.valueString(18))); // Recognition
																// Type
			a.setFileName(stringValue(query.valueString(19))); // File Name
			a.setAttachment(booleanValue(query.valueString(20).toString(),
					false));
	...
	public List<Resource> getNoteResourcesRecognition(String noteGuid) {
		...
		if (!check) {
			logger.log(logger.EXTREME,
					"NoteTable.getNoteRecognition SQL prepare has failed.");
			logger.log(logger.MEDIUM, query.lastError());
			return null;
		}
		...
		if (!check) {
			logger.log(logger.EXTREME,
					"NoteTable.getNoteRecognition exec has failed.");
			logger.log(logger.MEDIUM, query.lastError());
			return null;
		}
		while (query.next()) {
	...
	public void updateNoteResourceGuid(String oldGuid, String newGuid,
			boolean isDirty) {
		logger.log(logger.HIGH,
				"Entering RNoteResourceTable.updateNoteResourceGuid");
		NSqlQuery query = new NSqlQuery(db.getResourceConnection());
		...
		query.exec("commit");
		logger.log(logger.HIGH,
				"Leaving RNoteResourceTable.updateNoteResourceGuid");
	}
	...
	public void updateNoteSourceUrl(String guid, String url, boolean isDirty) {
		logger.log(logger.HIGH,
				"Entering RNoteResourceTable.updateNoteSourceUrl()");
		NSqlQuery query = new NSqlQuery(db.getResourceConnection());
		...
		query.exec("commit");
		logger.log(logger.HIGH,
				"Leaving RNoteResourceTable.updateNoteSourceUrl()");
	}

	// Get note source
	public String getNoteSourceUrl(String guid) {
		logger.log(logger.HIGH,
				"Entering RNoteResourceTable.getNoteSourceUrl()");
		NSqlQuery query = new NSqlQuery(db.getResourceConnection());
		...
		if (query.next()) {
			logger.log(logger.HIGH,
					"Leaving RNoteResourceTable.getNoteSourceUrl()");
			return query.valueString(0);
		}
		logger.log(logger.HIGH,
				"Leaving RNoteResourceTable.getNoteSourceUrl() - no value found");
		return null;
	}
	...
	public List<String> getDistinctNoteGuids() {
		logger.log(logger.HIGH,
				"Entering NoteResourceTable.getDistinctNoteGuids()");
		List<String> guids = new ArrayList<String>();
		NSqlQuery query = new NSqlQuery(db.getResourceConnection());
		...
		logger.log(logger.HIGH,
				"Leaving NoteResourceTable.getDistinctNoteGuids()");
		return guids;
	}
}
