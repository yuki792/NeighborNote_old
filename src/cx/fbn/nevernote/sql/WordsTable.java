...
public class WordsTable {
	...
	public void createTable() {
		...
		if (!query
				.exec("create table words (word varchar, guid varchar, source varchar, weight int, primary key (word, guid, source));")) {
	...
	public void expungeFromWordIndex(String guid, String type) {
		...
		if (!deleteWords
				.prepare("delete from words where guid=:guid and source=:source")) {
			logger.log(logger.EXTREME,
					"Note SQL select prepare deleteWords has failed.");
	...
	public void expunge(String guid) {
		...
		if (!deleteWords.prepare("delete from words where guid=:guid")) {
			logger.log(logger.EXTREME,
					"Word SQL select prepare expunge has failed.");
			logger.log(logger.MEDIUM, deleteWords.lastError());
		}
	...
	// Reindex a note
	public synchronized void addWordToNoteIndex(String guid, String word,
			String type, Integer weight) {
		NSqlQuery findWords = new NSqlQuery(db.getIndexConnection());
		if (!findWords
				.prepare("Select weight from words where guid=:guid and source=:type and word=:word")) {
		...
		findWords.exec();
		// If we have a match, find out which has the heigher weight & update
		// accordingly
		if (findWords.next()) {
			...
			if (recordWeight < weight) {
				...
				if (!updateWord
						.prepare("Update words set weight=:weight where guid=:guid and source=:type and word=:word")) {
					logger.log(logger.MEDIUM,
							"Prepare failed for find words in addWordToNoteIndex()");
					logger.log(logger.MEDIUM, findWords.lastError());
				}
		...
		if (!insertWords
				.prepare("Insert Into Words (word, guid, weight, source)"
						+ " Values(:word, :guid, :weight, :type )")) {
			logger.log(logger.EXTREME,
					"Note SQL select prepare checkWords has failed.");
		...
		if (!insertWords.exec()) {
			...
			logger.log(logger.MEDIUM, "Error inserting words into index: "
					+ err);
		}
	}
	...
	public List<String> getGuidList() {
		...
		if (!check)
			logger.log(logger.HIGH,
					"Table WORDS select distinct guid has FAILED!!!");
