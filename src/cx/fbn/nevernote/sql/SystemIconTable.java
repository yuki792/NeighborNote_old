...
public class SystemIconTable {
	...
	public void createTable() {
		...
		if (!query
				.exec("Create table SystemIcon (name varchar, "
						+ "type varchar, " + "primary key(name, type), "
						+ "icon blob)"))
	...
	public QIcon getIcon(String name, String type) {
		...
		if (!query
				.prepare("Select icon from SystemIcon where name=:name and type=:type"))
	...
	public void addIcon(String name, String rectype, QIcon icon, String filetype) {
		...
		if (icon == null) {
			...
			if (!query
					.prepare("Insert into SystemIcon (icon, name, type) values (:icon, :name, :type)"))
				logger.log(logger.EXTREME,
						"Error preparing notebook icon select.");
		...
		if (!query.exec())
			logger.log(logger.LOW,
					"Error setting system icon. " + query.lastError());
	}

	// Set the notebooks custom icon
	public void updateIcon(String name, String rectype, QIcon icon,
			String filetype) {
		NSqlQuery query = new NSqlQuery(db.getConnection());
		if (icon == null) {
			if (!query
					.prepare("delete from SystemIcon where name=:name and type=:type"))
				logger.log(logger.EXTREME,
						"Error preparing notebook icon select.");
		} else {
			if (!query
					.prepare("update SystemIcon set icon=:icon where name=:name and type=:type"))
				logger.log(logger.EXTREME,
						"Error preparing notebook icon select.");
			QBuffer buffer = new QBuffer();
			if (!buffer.open(QIODevice.OpenModeFlag.ReadWrite)) {
