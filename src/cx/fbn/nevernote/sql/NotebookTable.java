...
public class NotebookTable {
	...
	public void createTable(boolean addDefaulte) {
		...
		if (!query.exec("Create table " + dbName
				+ " (guid varchar primary key, " + "sequence integer, "
				+ "name varchar, " + "defaultNotebook varchar, "
				+ "serviceCreated timestamp, " + "serviceUpdated timestamp, "
				+ "published boolean, " + "isDirty boolean, "
				+ "autoEncrypt boolean, " + "local boolean, "
				+ "archived boolean)"))
		...
		query.prepare("Insert Into " + dbName
				...
				+ "isDirty, autoEncrypt, " + "local, archived) Values("
				+ ":guid, :sequence, :name, :defaultNotebook,  "
				+ ":serviceCreated, :serviceUpdated, :published, "
				+ ":isDirty, :autoEncrypt, " + ":local, false)");
		...
		StringBuilder serviceCreated = new StringBuilder(simple.format(newnote
				.getServiceCreated()));
		StringBuilder serviceUpdated = new StringBuilder(simple.format(newnote
				.getServiceUpdated()));
		...
		if (!check) {
			logger.log(logger.MEDIUM, "Initial " + dbName
					+ " Table insert failed.");
	...
	public void addNotebook(Notebook tempNotebook, boolean isDirty,
			boolean local) {
	...
	// Save an individual notebook
	public void addNotebook(Notebook tempNotebook, boolean isDirty,
			boolean local, boolean linked, boolean readOnly) {
		boolean check;
		...
		StringBuilder serviceCreated = new StringBuilder(
				simple.format(tempNotebook.getServiceCreated()));
		StringBuilder serviceUpdated = new StringBuilder(
				simple.format(tempNotebook.getServiceUpdated()));
		if (serviceUpdated.toString() == null)
			serviceUpdated = serviceCreated;
		query.bindValue(":serviceCreated", serviceCreated.toString());
		query.bindValue(":serviceUpdated", serviceCreated.toString());
	...
	public void expungeNotebook(String guid, boolean needsSync) {
		...
		if (!check) {
			logger.log(logger.EXTREME, dbName
					+ " SQL delete prepare has failed.");
	...
	public void updateNotebook(Notebook tempNotebook, boolean isDirty) {
		...
		check = query
				.prepare("Update "
						+ dbName
						+ " set sequence=:sequence, name=:name, defaultNotebook=:defaultNotebook, "
		...
		if (tempNotebook.isPublished()) {
			...
			query.bindValue(":order", tempNotebook.getPublishing().getOrder()
					.getValue());
			query.bindValue(":ascending", tempNotebook.getPublishing()
					.isAscending());
			query.bindValue(":desc", tempNotebook.getPublishing()
	...
	public List<Notebook> getAll() {
		...
		check = query
				.exec("Select guid, sequence, name, defaultNotebook, "
						+ "serviceCreated, "
						+ "serviceUpdated, "
		...
		while (query.next()) {
			...
			try {
				tempNotebook.setServiceCreated(indfm
						.parse(query.valueString(4)).getTime());
				tempNotebook.setServiceUpdated(indfm
						.parse(query.valueString(5)).getTime());
	...
	public List<Notebook> getAllLocal() {
		...
		check = query.exec("Select guid, sequence, name, defaultNotebook, "
				+ "serviceCreated, serviceUpdated, published, stack from "
				+ dbName + " where local=true order by name");
		if (!check)
			logger.log(logger.EXTREME, dbName + " SQL retrieve has failed.");
		while (query.next()) {
			tempNotebook = new Notebook();
	...
	public void setArchived(String guid, boolean val) {
		boolean check;
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query.prepare("Update " + dbName
				+ " set archived=:archived where guid=:guid");
		if (!check)
			logger.log(logger.EXTREME, dbName
					+ " SQL archive update has failed.");
		query.bindValue(":guid", guid);
	...
	public List<Notebook> getAllArchived() {
		...
		check = query
				.exec("Select guid, sequence, name, defaultNotebook, "
						...
						+ "publishingOrder " + "from " + dbName
						+ " where archived=true order by name");
		if (!check)
			logger.log(logger.EXTREME, dbName + " SQL retrieve has failed.");
		while (query.next()) {
			tempNotebook = new Notebook();
	...
	public boolean isReadOnly(String guid) {
		...
		query.prepare("Select readOnly from " + dbName
				+ " where guid=:guid and readOnly=true");
		query.bindValue(":guid", guid);
		query.exec();
		if (!query.next()) {
			return false;
	...
	public void updateNotebookSequence(String guid, int sequence) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query.prepare("Update " + dbName
				+ " set sequence=:sequence where guid=:guid");
		query.bindValue(":guid", guid);
	...
	public List<Notebook> getDirty() {
		...
		check = query
				.exec("Select guid, sequence, name, defaultNotebook, "
						...
						+ "from "
						+ dbName
						+ " where isDirty=true and local=false and linked=false");
		if (!check)
			logger.log(logger.EXTREME, dbName + " SQL retrieve has failed.");
		while (query.next()) {
			tempNotebook = new Notebook();
			...
			if (tempNotebook.isPublished()) {
				...
				p.setPublicDescription(query.valueString(10));
				p.setOrder(NoteSortOrder.findByValue(query.valueInteger(11)));
				if (p.getPublicDescription() != null
						&& p.getPublicDescription().trim().equalsIgnoreCase(""))
	...
	public Notebook getNotebook(String guid) {
		...
		query.prepare("Select guid, sequence, name, defaultNotebook, "
				+ "serviceCreated, serviceUpdated, published, stack, "
				+ "publishinguri, publishingascending, publishingPublicDescription, "
				+ "publishingOrder " + "from " + dbName + " where guid=:guid");
		query.bindValue(":guid", guid);
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME, dbName + " SQL retrieve has failed.");
	...
	// This is a convience method to check if a tag exists & update/create based
	// upon it
	...
	// This is a convience method to check if a tag exists & update/create based
	// upon it
	public void syncLinkedNotebook(Notebook notebook, boolean isDirty,
			boolean readOnly) {
		if (!exists(notebook.getGuid())) {
	...
	public void resetDirtyFlag(String guid) {
		...
		query.prepare("Update " + dbName
				+ " set isdirty='false' where guid=:guid");
		query.bindValue(":guid", guid);
		if (!query.exec())
			logger.log(logger.EXTREME, "Error resetting " + dbName
					+ " dirty field.");
	}
	...
	public void setDefaultNotebook(String guid) {
		...
		query.prepare("Update "
				+ dbName
				+ " set defaultNotebook=false, isDirty=true where linked=false and defaultNotebook=true");
		if (!query.exec())
			logger.log(logger.EXTREME, "Error removing default " + dbName + ".");
		query.prepare("Update "
				+ dbName
				+ " set defaultNotebook=true, isDirty=true where guid=:guid and linked=false");
		query.bindValue(":guid", guid);
		if (!query.exec())
	...
	public HashMap<String, QIcon> getAllIcons() {
		...
		if (!query.exec("SELECT guid, icon from " + dbName
				+ " where ARCHIVED  != true"))
			logger.log(logger.EXTREME, "Error executing " + dbName
					+ " getAllIcons select.");
		while (query.next()) {
	...
	public QIcon getIcon(String guid) {
		...
		if (!query.prepare("Select icon from " + dbName + " where guid=:guid"))
			logger.log(logger.EXTREME, "Error preparing " + dbName
					+ " icon select.");
		query.bindValue(":guid", guid);
		if (!query.exec())
	...
	public void setIcon(String guid, QIcon icon, String type) {
		NSqlQuery query = new NSqlQuery(db.getConnection());
		if (icon == null) {
			if (!query.prepare("update " + dbName
					+ " set icon=null where guid=:guid"))
				logger.log(logger.EXTREME, "Error preparing " + dbName
						+ " icon select.");
		} else {
			if (!query.prepare("update " + dbName
					+ " set icon=:icon where guid=:guid"))
				logger.log(logger.EXTREME, "Error preparing " + dbName
						+ " icon select.");
		...
		query.bindValue(":guid", guid);
		if (!query.exec())
			logger.log(logger.LOW, "Error setting " + dbName + " icon. "
					+ query.lastError());
	}
	...
	public void setReadOnly(String guid, boolean readOnly) {
		NSqlQuery query = new NSqlQuery(db.getConnection());
		if (!query.prepare("update " + dbName
				+ " set readOnly=:readOnly where guid=:guid"))
			logger.log(logger.EXTREME, "Error preparing " + dbName
					+ " read only.");
		query.bindValue(":guid", guid);
		query.bindValue(":readOnly", readOnly);
		if (!query.exec())
			logger.log(logger.LOW, "Error setting " + dbName + " read only. "
					+ query.lastError());
	}
	...
	public List<Pair<String, Integer>> getNotebookCounts() {
		...
		if (notebookCountQuery == null) {
			...
			notebookCountQuery
					.prepare("select notebookGuid, count(guid) from note where active=1 group by notebookguid;");
		}
	...
	public void setStack(String guid, String stack) {
		...
		query.prepare("Update " + dbName
				+ " set stack=:stack, isDirty=true where guid=:guid");
		query.bindValue(":guid", guid);
	...
	public List<String> getAllStackNames() {
		...
		while (query.next()) {
			if (query.valueString(0) != null
					&& !query.valueString(0).trim().equals(""))
	...
	public void renameStacks(String oldName, String newName) {
		...
		if (!query.prepare("update " + dbName
				+ " set stack=:newName where stack=:oldName")) {
		...
		if (!query
				.prepare("update SystemIcon set name=:newName where name=:oldName and type='STACK'")) {
			logger.log(logger.EXTREME,
					"Error preparing icon rename in renameStacks.");
			return;
		}
		query.bindValue(":oldName", oldName);
		query.bindValue(":newName", newName);
		if (!query.exec()) {
			logger.log(logger.EXTREME,
					"Error updating stack names for SystemIcon");
			return;
		}
	...
	public boolean stackExists(String stack) {
		...
		query.prepare("Select guid from " + dbName
				+ " where stack=:stack limit 1");
		query.bindValue(":stack", stack);
		if (!query.exec())
	...
	public void setPublishing(String guid, boolean published, Publishing p) {
		...
		query.prepare("Update "
				+ dbName
				+ " set publishingPublicDescription=:publishingPublicDescription, "
	...
	public String getNotebookByUri(String uri) {
		...
		check = query.prepare("Select guid " + "from " + dbName
				+ " where publishingUri=:uri");
		query.bindValue(":uri", uri);
		check = query.exec();
		if (!check)
			logger.log(logger.EXTREME,
					"Notebook SQL retrieve guid by uri has failed.");
		if (query.next()) {
	...
	public int getSortColumn(String guid) {
		...
		if (Global.getSortOrder() != Global.View_List_Wide)
			check = query.prepare("Select wide_sort_column " + "from " + dbName
					+ " where guid=:guid");
		else
			check = query.prepare("Select narrow_sort_column " + "from "
					+ dbName + " where guid=:guid");
		query.bindValue(":guid", guid);
		check = query.exec();
		if (!check) {
			logger.log(logger.EXTREME,
					"Notebook SQL retrieve sort order has failed.");
			return -1;
		}
		if (query.next()) {
	...
	public int getSortOrder(String guid) {
		...
		if (Global.getSortOrder() != Global.View_List_Wide)
			check = query.prepare("Select wide_sort_order " + "from " + dbName
					+ " where guid=:guid");
		else
			check = query.prepare("Select narrow_sort_order " + "from "
					+ dbName + " where guid=:guid");
		query.bindValue(":guid", guid);
		check = query.exec();
		if (!check) {
	...
	public void setSortOrder(String guid, int column, int order) {
		...
		if (Global.getSortOrder() != Global.View_List_Wide)
			check = query
					.prepare("Update "
							+ dbName
							+ " set wide_sort_order=:order, wide_sort_column=:column where guid=:guid");
		else
			check = query
					.prepare("Update "
							+ dbName
							+ " set narrow_sort_order=:order, narrow_sort_column=:column where guid=:guid");
		...
		if (!check)
			logger.log(logger.EXTREME,
					"Notebook SQL set sort order has failed.");
	}
	...
	public boolean isLinked(String guid) {
		...
		check = query.prepare("Select guid " + "from " + dbName
				+ " where guid=:guid and linked=true");
		query.bindValue(":guid", guid);
		check = query.exec();
		if (!check)
	...
	public void convertFromSharedNotebook(String guid, boolean local) {
		...
		if (!query.exec())
			logger.log(logger.EXTREME,
					"NotebookTable.convertToLocalNotebook error.");
		...
		if (!query.exec())
			logger.log(logger.EXTREME,
					"NotebookTable.convertToLocalNotebook #2 error.");
