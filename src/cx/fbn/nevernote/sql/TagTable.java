...
public class TagTable {
	...
	public void createTable() {
		...
		if (!query
				.exec("Create table Tag (guid varchar primary key, "
	...
	public List<Tag> getTagsForNotebook(String notebookGuid) {
		...
		if (!check) {
			logger.log(logger.EXTREME,
					"Tag SQL prepare getTagsForNotebook has failed.");
	...
	public List<Tag> getAll() {
		...
		check = query
				.exec("Select guid, parentGuid, sequence, name"
	...
	public Tag getTag(String guid) {
		...
		if (!query.prepare("Select guid, parentGuid, sequence, name"
			...
			logger.log(logger.EXTREME,
					"Tag select by guid SQL prepare has failed.");

		query.bindValue(":guid", guid);
		if (!query.exec())
			logger.log(logger.EXTREME,
					"Tag select by guid SQL exec has failed.");
	...
	public void updateTag(Tag tempTag, boolean isDirty, String realName) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Update Tag set parentGuid=:parentGuid, sequence=:sequence, "
	...
	public void expungeTag(String guid, boolean needsSync) {
		...
		check = query.prepare("delete from Tag " + "where guid=:guid");
		if (!check) {
		...
		if (!check) {
			logger.log(logger.EXTREME,
					"NoteTags SQL delete prepare has failed.");
			logger.log(logger.EXTREME, query.lastError());
		}
	...
	// Save a tag
	public void addTag(Tag tempTag, boolean isDirty, boolean isLinked,
			String realName, String notebookGuid) {
		boolean check;

		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Insert Into Tag (guid, parentGuid, sequence, hashCode, name, isDirty, linked, realName, notebookGuid)"
	...
	public void updateTagParent(String guid, String parentGuid) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Update Tag set parentGuid=:parentGuid where guid=:guid");
		if (!check) {
			logger.log(logger.EXTREME,
					"Tag SQL tag parent update prepare has failed.");
			logger.log(logger.EXTREME, query.lastError());
		}
	...
	public void updateTagSequence(String guid, int sequence) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Update Tag set sequence=:sequence where guid=:guid");
	...
	public void updateTagGuid(String oldGuid, String newGuid) {
		...
		NSqlQuery query = new NSqlQuery(db.getConnection());
		check = query
				.prepare("Update Tag set guid=:newGuid where guid=:oldGuid");
		...
		check = query
				.prepare("Update Tag set parentGuid=:newGuid where parentGuid=:oldGuid");
		query.bindValue(":newGuid", newGuid);
		query.bindValue(":oldGuid", oldGuid);
		query.exec();
		if (!check) {
		...
		check = query
				.prepare("Update NoteTags set tagGuid=:newGuid where tagGuid=:oldGuid");
		query.bindValue(":newGuid", newGuid);
		query.bindValue(":oldGuid", oldGuid);
		query.exec();
		if (!check) {
	...
	public List<Tag> getDirty() {
		...
		while (query.next()) {
			...
			tempTag.setUpdateSequenceNum(sequence);
			tempTag.setName(query.valueString(3));
			if (tempTag.getParentGuid() != null
					&& tempTag.getParentGuid().equals(""))
				tempTag.setParentGuid(null);
			index.add(tempTag);
		}
		return index;
	...
	public void setIcon(String guid, QIcon icon, String type) {
		...
		if (!query.exec())
			logger.log(logger.LOW,
					"Error setting tag icon. " + query.lastError());
	}
	...
	public HashMap<String, QIcon> getAllIcons() {
		...
		if (!query.exec("SELECT guid, icon from tag"))
			logger.log(logger.EXTREME,
					"Error executing SavedSearch getAllIcons select.");
		while (query.next()) {
	...
	public List<String> findChildren(String guid, List<Tag> tagList) {
		...
		for (int i = 0; i < tagList.size(); i++) {
			if (tagList.get(i).getParentGuid().equalsIgnoreCase(guid)) {
				...
				List<String> childMatch = findChildren(
						tagList.get(i).getGuid(), tagList);
