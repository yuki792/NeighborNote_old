...
public class DatabaseConnection {
	...
	public DatabaseConnection(ApplicationLogger l, String url, String iurl,
			String rurl, String burl, String userid, String password,
			String cypherPassword, int throttle) {
		logger = l;
	...
	public void dbSetup(String url, String indexUrl, String resourceUrl,
			String behaviorUrl, String userid, String userPassword,
		...
		f = Global.getFileManager().getDbDirFile(
		...
		try {
			...
			// conn = DriverManager.getConnection(url,userid,passwordString);
			// conn =
			// DriverManager.getConnection(url+";CACHE_SIZE=4096",userid,passwordString);
			if (throttle == 0) {
				conn = DriverManager.getConnection(url + ";CACHE_SIZE="
						+ Global.databaseCache, userid, passwordString);
			} else {
				conn = DriverManager.getConnection(url + ";THROTTLE="
						+ new Integer(throttle).toString() + ";CACHE_SIZE="
						+ Global.databaseCache, userid, passwordString);
			}
			indexConn = DriverManager.getConnection(indexUrl, userid,
					passwordString);
			resourceConn = DriverManager.getConnection(resourceUrl, userid,
					passwordString);
			// ICHANGED
			...
			// conn =
			// DriverManager.getConnection(url+";AUTO_SERVER=TRUE",userid,passwordString);
		} catch (SQLException e) {
			e.printStackTrace();
			return;
		}

		// If it doesn't exist and we are the main thread, then we need to
		// create stuff.
		if (!dbExists) {
			createTables();
		...
		if (!resourceDbExists) {
			...
			if (dbTableExists("NoteResources")) {
				...
				String linkcmd = "create linked table oldnoteresources "
						+ "('org.h2.Driver', '" + url + "', '" + userid
						+ "', '" + passwordString + "', 'NoteResources')";
				query.exec(linkcmd);
		...
		// If we encrypted/decrypted it the last time, we need to reconnect the
		// tables.
		...
		// +"('org.h2.Driver', '"+url+"', '"+userid+"', '"+passwordString+
		// "', 'NoteResources')";
	...
	public void beginTransaction() {
		...
		if (!query.exec("Begin Transaction"))
			logger.log(logger.EXTREME,
					"Begin transaction has failed: " + query.lastError());
	...
	public void commitTransaction() {
		...
		if (!query.exec("Commit"))
			logger.log(logger.EXTREME, "Transaction commit has failed: "
					+ query.lastError());
	}
