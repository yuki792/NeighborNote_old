...
public class InvalidXMLTable {
	...
	public void createTable() {
		...
		if (!query
				.exec("Create table InvalidXML (type varchar, element varchar, attribute varchar,primary key(type, element,attribute) );"))
	...
	public void addAttribute(String element, String attribute) {
		...
		if (!query.exec()) {
			logger.log(logger.MEDIUM,
					"Insert Attribute into invalidXML failed.");
	...
	public List<String> getInvalidElements() {
		NSqlQuery query = new NSqlQuery(db.getConnection());
		if (!query
				.exec("Select element from InvalidXML where type = 'ELEMENT'")) {
			logger.log(logger.MEDIUM,
					"getInvalidElement from invalidXML failed.");
			logger.log(logger.MEDIUM, query.lastError());
			return null;
		}
	...
	public List<String> getInvalidAttributeElements() {
		NSqlQuery query = new NSqlQuery(db.getConnection());
		if (!query
				.exec("Select distinct element from InvalidXML where type = 'ATTRIBUTE'")) {
			logger.log(logger.MEDIUM,
					"getInvalidElement from invalidXML failed.");
			logger.log(logger.MEDIUM, query.lastError());
			return null;
