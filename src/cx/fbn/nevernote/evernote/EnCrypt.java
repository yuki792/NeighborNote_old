...
public class EnCrypt {
	 ...
	 * Main changes are
	 * 
	 * 1. Do padding based on encoded bytes, not string length (some chars -> 2
	 * bytes) 2. Use specific named charset
	 */
	private byte[] encodeStringNew(String text) {

		final Charset charSet = getCharset();

		// Convert to bytes using given encoding, and align *bytes* to multiple
		// of
		...
		final byte[] paddedBytes = Arrays.copyOf(bytes, bytes.length
				+ paddingNeeded);
		...
		System.arraycopy(paddedBytes, 0, total, crcBytes.length,
				paddedBytes.length);
	...
	private String decodeBytesNew(byte[] bytes) {
		...
		if (realCRC.equals(cryptCRC)) {
			// Trim nulls at end
			while (textChar.get(textChar.limit() - 1) == 0
					&& textChar.limit() != 0) {
	...
	private String decodeBytesOld(byte[] bytes) {
		...
		// We have a result. Separate it into the 4 byte header and the
		// decrypted text
	...
	public String decrypt(String text, String passphrase, int keylen) {
		...
		try {
			...
			// System.out.println("Are same decrypted ? " +
			// clearTextNew.equals(clearTextOld));
			// }
			return clearTextNew;
		} catch (NoSuchAlgorithmException e) {
			e.printStackTrace();
