...
public class EnmlConverter {
	...
	public String convert(String noteGuid, String content) {
		...
		// someplace other than the editor (for example, if we are merging
		// notes).
		...
		// Fix the more common XML problems that Webkit creates, but are not
		// considered
		// valid XML.
		...
		if (tidyContent != null) {
			...
			logger.log(logger.HIGH,
					"Error converting to JTidy.  Falling back to old method");
			...
			if (repairedContent == null) {
				...
				logger.log(logger.LOW,
						"Parse error when converting to ENML. Aborting save");
				return null;
			}
		...
		// also contains the 3 other PUBLIC definitions at the beginning of the
		// dtd.
	...
	private String fixEncryptionTags(String content) {
		...
		for (int i = newContent.indexOf(eTag); i > 0; i = newContent.indexOf(
				eTag, i + 1)) {
	...
	public String fixStupidXMLProblems(String content) {
		...
		for (int i = newContent.indexOf("<img"); i > 0; i = newContent.indexOf(
				"<img", i + 1)) {
		...
		for (int i = newContent.indexOf("<input"); i > 0; i = newContent
				.indexOf("<input", i + 1)) {
			endPos = newContent.indexOf(">", i + 1);
			String end = newContent.substring(endPos + 1);
			newContent = newContent.subSequence(0, endPos) + "/>" + end;
		...
		for (int i = newContent.indexOf("<br"); i > 0; i = newContent.indexOf(
				"<br", i + 1)) {
			endPos = newContent.indexOf(">", i + 1);
			String end = newContent.substring(endPos + 1);
			newContent = newContent.subSequence(0, endPos) + "/>" + end;
		...
		for (int i = newContent.indexOf("<hr"); i > 0; i = newContent.indexOf(
				"<hr", i + 1)) {
			endPos = newContent.indexOf(">", i + 1);
			String end = newContent.substring(endPos + 1);
			newContent = newContent.subSequence(0, endPos) + "/>" + end;
		...
		for (int i = newContent.indexOf("<meta"); i > 0; i = newContent
				.indexOf("<meta", i + 1)) {
			endPos = newContent.indexOf(">", i + 1);
			String end = newContent.substring(endPos + 1);
			newContent = newContent.subSequence(0, endPos) + "/>" + end;
	...
	public String fixEnXMLCrap(String note) {
		...
		// change all <b/> to <b></b> because Evernote hates them if they happen
		// in <span>
		...
		// change all <br/> to <br></br> because Evernote hates them if they
		// happen in <span>
		...
		// change all <span> elements in lists because Evernote hates them if
		// they happen
		int endPos = 0;
		int spanPos;
