...
public class ImportData {
	...
	public void importData(String f) {
		...
		while (!reader.atEnd()) {
			...
			if (reader.name().equalsIgnoreCase("nevernote-export")
				...
				if (!version.equalsIgnoreCase("0.85")
						&& !version.equalsIgnoreCase("0.86")
				...
				if (!application.equalsIgnoreCase("NeverNote")) {
					lastError = 2;
					errorMessage = "This backup is from an unknown application = "
							+ application;
					return;
				}
			...
			if (reader.name().equalsIgnoreCase("Synchronization")
					&& reader.isStartElement() && backup) {
			...
			if (reader.name().equalsIgnoreCase("note")
				...
				if (metaData.containsKey(note.getGuid()))
					conn.getNoteTable().updateNoteMetadata(
							metaData.get(note.getGuid()));
			}
			if (reader.name().equalsIgnoreCase("notebook")
					&& reader.isStartElement() && (backup || importNotebooks)) {
				processNotebookNode();
				String existingGuid = conn.getNotebookTable()
						.findNotebookByName(notebook.getName());
				if (existingGuid == null) {
					conn.getNotebookTable().addNotebook(notebook,
							notebookIsDirty, notebookIsLocal);
				} else {
					conn.getNotebookTable().updateNotebookGuid(existingGuid,
							notebook.getGuid());
					conn.getNotebookTable().updateNotebook(notebook,
							notebookIsDirty);
				}
				conn.getNotebookTable().setIcon(notebook.getGuid(),
						notebookIcon, "PNG");
				conn.getNotebookTable().setReadOnly(notebook.getGuid(),
						notebookIsReadOnly);
			}
			if (reader.name().equalsIgnoreCase("tag")
					&& reader.isStartElement() && (backup || importTags)) {
				processTagNode();
				String testGuid = conn.getTagTable().findTagByName(
						tag.getName());
				if (testGuid == null)
			...
			if (reader.name().equalsIgnoreCase("savedsearch")
					&& reader.isStartElement() && backup) {
				processSavedSearchNode();
				conn.getSavedSearchTable()
						.addSavedSearch(search, searchIsDirty);
			}
			if (reader.name().equalsIgnoreCase("LinkedNotebook")
					&& reader.isStartElement() && backup) {
				processLinkedNotebookNode();
				conn.getLinkedNotebookTable().addNotebook(linkedNotebook,
						linkedNotebookIsDirty);
			}
			if (reader.name().equalsIgnoreCase("SharedNotebook")
					&& reader.isStartElement() && backup) {
				processSharedNotebookNode();
				conn.getSharedNotebookTable().addNotebook(sharedNotebook,
						sharedNotebookIsDirty);
			}
		}
		xmlFile.close();
	}
			...
			if (reader.isStartElement()) {
				if (reader.name().equalsIgnoreCase("Guid")) {
					...
					if (!backup) {
						...
						String newGuid = "IMP"
								+ new Integer(random1.nextInt(1000)).toString();
						newGuid = newGuid + "-"
								+ new Integer(random1.nextInt(1000)).toString();
						newGuid = newGuid + "-"
								+ new Integer(random1.nextInt(1000)).toString();
	...
	private Resource processResource() {
		...
		conn.getNoteTable().noteResourceTable.saveNoteResource(resource,
				isDirty);
			...
			if (backup || importNotebooks) {
				if (reader.isStartElement()) {
					...
					if (reader.name().equalsIgnoreCase(
						...
						notebook.getPublishing().setPublicDescription(
								textValue());
					}
					...
					if (reader.name().equalsIgnoreCase("PublishingOrder")) {
						notebook.getPublishing().setOrder(
								NoteSortOrder.findByValue(intValue()));
					}
					if (reader.name().equalsIgnoreCase("ReadOnly")) {
						if (booleanValue())
							notebookIsReadOnly = true;
					...
					if (reader.name().equalsIgnoreCase("Icon")) {
						...
						QByteArray binData = new QByteArray(
								QByteArray.fromHex(hexData));
						notebookIcon = new QIcon(QPixmap.fromImage(QImage
								.fromData(binData)));
					}
