...
public class NoteFormatter {
	...
	public NoteFormatter(ApplicationLogger logger, DatabaseConnection conn,
			List<QTemporaryFile> tempFiles2) {
		this.logger = logger;
		this.conn = conn;
		noteHistory = false;
	}
	...
	public void setNote(Note note, boolean pdfPreview) {
		...
		if (note != null) {
			...
			readOnly = conn.getNotebookTable().isReadOnly(
	...
	public void setHighlight(EnSearch search) {
		if (search == null || search.hilightWords == null
				|| search.hilightWords.size() == 0) {
			enSearch = null;
			addHighlight = false;
		} else {
			enSearch = search;
	...
	// Set if we are coming here through note histary. It triggers longer file
	...
	public String rebuildNoteHTML() {
		...
		if (!result.success) {
			logger.log(logger.LOW,
					"Error parsing document.  Attempting to restructure");
			Tidy tidy = new Tidy();
		...
		if (!result.success) {
			...
			logger.log(logger.MEDIUM, "Line: " + result.errorLine + " Column: "
					+ result.errorColumn);
			System.out.println("Error: " + result.errorMessage);
			System.out.println("Line: " + result.errorLine + " Column: "
					+ result.errorColumn);
		...
		// Fix the stupid problem where inserting an <img> tag after an <a> tag
		// (which is done
		// to get the <en-media> application tag to work properly) causes spaces
		// to be inserted
		// between the <a> & <img>. This messes things up later. This is an ugly
		// hack.
		StringBuffer html = new StringBuffer(doc.toString());
		for (int i = html.indexOf("<a en-tag=\"en-media\" ", 0); i > -1; i = html
				.indexOf("<a en-tag=\"en-media\" ", i)) {
			i = html.indexOf(">\n", i + 1);
	...
	private void addImageHilight(String resGuid, QFile f) {
		if (enSearch == null || enSearch.hilightWords == null
				|| enSearch.hilightWords.size() == 0)
			return;

		// Get the recognition XML that tells where to hilight on the image
		Resource recoResource = conn.getNoteTable().noteResourceTable
				.getNoteResourceRecognition(resGuid);
		if (recoResource.getRecognition().getBody() == null
				|| recoResource.getRecognition().getBody().length == 0)
			return;
		QByteArray recoData = new QByteArray(recoResource.getRecognition()
				.getBody());
		String xml = recoData.toString();

		// Get a painter for the image. This is the background (the initial
		// image).
		...
		// piece is the hilight that will be overlayed to hilight text no the
		// background
		...
		for (int i = 0; i < anchors.length(); i++) {
			...
			QDomNodeList children = element.childNodes(); // all children ("t"
															// nodes).
			...
			for (int j = 0; j < children.length(); j++) {
				...
				if (child.nodeName().equalsIgnoreCase("t")) {
					...
					int weight = new Integer(child.attribute("w")); // recognition
																	// weight
					if (weight >= Global.getRecognitionWeight()) { // Are we
																	// above the
																	// maximum?

						// Check to see if this word matches something we were
						// searching for.
						for (int k = 0; k < enSearch.hilightWords.size(); k++) {
							String searchWord = enSearch.hilightWords.get(k)
									.toLowerCase();
							if (searchWord.startsWith("*"))
								searchWord = searchWord.substring(1);
							if (searchWord.endsWith("*"))
								searchWord = searchWord.substring(0,
										searchWord.length() - 1);
	...
	private void modifyImageTags(QDomDocument doc, QDomElement docElem,
			QDomElement enmedia, QDomAttr hash) {
		...
		String resGuid = conn.getNoteTable().noteResourceTable
				.getNoteResourceGuidByHashHex(currentNoteGuid, hash.value());
		QFile tfile = new QFile(Global.getFileManager().getResDirPath(
				resGuid + type));
		// if (!tfile.exists()) {
		Resource r = null;
		if (resGuid != null)
			r = conn.getNoteTable().noteResourceTable.getNoteResource(resGuid,
					true);
		if (r == null || r.getData() == null
				|| r.getData().getBody().length == 0) {
			resourceError = true;
			readOnly = true;
		}
		if (r != null && r.getData() != null
			...
			enmedia.setAttribute("src", QUrl.fromLocalFile(tfile.fileName())
					.toString());
		...
		// }
		// Technically, we should do a file:// to have a proper url, but for
		// some reason QWebPage hates
		// them and won't generate a thumbnail image properly if we use them.
		// enmedia.setAttribute("src",
		// QUrl.fromLocalFile(tfile.fileName()).toString());
		enmedia.setAttribute("src", tfile.fileName().toString());
		enmedia.setAttribute("en-tag", "en-media");
		enmedia.setTagName("img");
		if (r != null
				&& r.getAttributes() != null
				&& (r.getAttributes().getSourceURL() == null || !r
						.getAttributes().getSourceURL().toLowerCase()
						.startsWith("http://latex.codecogs.com/gif.latex?")))
			enmedia.setAttribute("onContextMenu",
					"window.jambi.imageContextMenu('" + tfile.fileName()
							+ "');");
		else {
			...
			if (r != null && r.getAttributes() != null
					&& r.getAttributes().getSourceURL() != null)
				newText.setAttribute("title", r.getAttributes().getSourceURL());
			newText.setAttribute("href", "latex://"
					+ tfile.fileName().toString());
	...
	private QDomDocument modifyTags(QDomDocument doc) {
		...
		for (int i = enCryptLen - 1; i >= 0; i--) {
			...
			enmedia.setAttribute("src", Global.getFileManager()
			...
			// If the encryption string contains crlf at the end, remove them
			// because they mess up the javascript.
			if (encryptedText.endsWith("\n"))
				encryptedText = encryptedText.substring(0,
			...
			hint = hint.replace("'", "&apos;");
			enmedia.setAttribute("onClick", "window.jambi.decryptText('crypt"
					+ Global.cryptCounter.toString() + "', '" + encryptedText
					+ "', '" + hint + "');");
			enmedia.setAttribute("onMouseOver", "style.cursor='hand'");
			enmedia.setTagName("img");
			enmedia.removeChild(enmedia.firstChild()); // Remove the actual
														// encrypted text
		}
	...
	private boolean buildInkNote(QDomDocument doc, QDomElement docElem,
			QDomElement enmedia, QDomAttr hash, String appl) {
		String resGuid = conn.getNoteTable().noteResourceTable
				.getNoteResourceGuidByHashHex(currentNote.getGuid(),
						hash.value());
		Resource r = conn.getNoteTable().noteResourceTable.getNoteResource(
				resGuid, false);

		// If we can't find the resource, then fall back to the old method.
		// We'll return & show
		// an error later
		if (r == null || r.getData() == null)
			return false;
		...
		if (!resourceError) {
			...
			// Get a list of images in the database. We'll use these to bulid
			// the page.
			List<QByteArray> data = conn.getInkImagesTable().getImage(
					r.getGuid());
			...
			// We have pictures, so append them to the page. This really isn't
			// proper since
			// we leave the en-media tag in place, but since we can't edit the
			// page it doesn't
			// hurt anything.
			for (int i = 0; i < data.size(); i++) {
				QFile f = new QFile(Global.getFileManager().getResDirPath(
						resGuid + new Integer(i).toString() + ".png"));
	...
	private void modifyApplicationTags(QDomDocument doc, QDomElement docElem,
			QDomElement enmedia, QDomAttr hash, String appl) {
		...
		if (r != null) {
			if (r.getData() != null) {
				...
				if (r.getAttributes() != null
						&& r.getAttributes().getFileName() != null
						&& !r.getAttributes().getFileName().equals(""))
				...
				if (fileDetails != null && !fileDetails.equals("")) {
					if (!noteHistory) {
						...
						enmedia.setAttribute("href", "nnres://" + r.getGuid()
								+ currentNote.getUpdateSequenceNum()
								+ Global.attachmentNameDelimeter + fileDetails);
						contextFileName = fileManager.getResDirPath(r.getGuid()
								+ currentNote.getUpdateSequenceNum()
								+ Global.attachmentNameDelimeter + fileDetails);
				...
				enmedia.setAttribute("onContextMenu",
						"window.jambi.resourceContextMenu('" + contextFileName
								+ "');");
				if (fileDetails == null || fileDetails.equals(""))
					fileDetails = "";
				enmedia.setAttribute("en-tag", "en-media");
				...
				if (appl.equalsIgnoreCase("pdf") && pdfPreview) {
					String fileName;
					Resource res = conn.getNoteTable().noteResourceTable
							.getNoteResource(r.getGuid(), true);
					if (res.getAttributes() != null
							&& res.getAttributes().getFileName() != null
							&& !res.getAttributes().getFileName().trim()
									.equals(""))
						fileName = res.getGuid()
					...
					goodPreview = pdfPreview.setupPreview(file.fileName(),
							appl, 0);
					if (goodPreview) {
						...
						left.setAttribute("onMouseDown",
								"window.jambi.nextPage('" + file.fileName()
										+ "')");
						left.setAttribute("onMouseDown",
								"window.jambi.nextPage('" + file.fileName()
										+ "')");
						...
						right.setAttribute("onMouseDown",
								"window.jambi.nextPage('" + file.fileName()
										+ "')");
						left.setAttribute("onMouseDown",
								"window.jambi.previousPage('" + file.fileName()
										+ "')");
						// NFC TODO: should these be file:// URLs?
						left.setAttribute("src", Global.getFileManager()
								.getImageDirPath("small_left.png"));
						right.setAttribute("src", Global.getFileManager()
								.getImageDirPath("small_right.png"));
				...
				if (icon.equals("attachment.png"))
					icon = findIcon(fileDetails.substring(fileDetails
							.indexOf(".") + 1));
				// NFC TODO: should this be a 'file://' URL?
				newText.setAttribute("src", Global.getFileManager()
						.getImageDirPath(icon));
				if (goodPreview) {
					// NFC TODO: should this be a 'file://' URL?
					newText.setAttribute("src",
							fileManager.getResDirPathSpecialChar(filePath));
					newText.setAttribute("style",
							"border-style:solid; border-color:green; padding:0.5mm 0.5mm 0.5mm 0.5mm;");
				}
	...
	private void modifyTodoTags(QDomElement todo) {
		...
		todo.setAttribute("onClick",
				"value=checked;window.jambi.contentChanged(); ");
	...
	public String modifyCachedTodoTags(String note) {
		...
		for (int i = html.indexOf("<input", 0); i > -1; i = html.indexOf(
				"<input", i)) {
	...
	public QDomDocument addHilight(QDomDocument doc) {
		...
		if (enSearch.hilightWords == null || enSearch.hilightWords.size() == 0)
			return doc;
		XMLInsertHilight hilight = new XMLInsertHilight(doc,
				enSearch.hilightWords);
