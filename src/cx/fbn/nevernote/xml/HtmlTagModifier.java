		...
		for (; position < html.length();) {
			...
			if (position > 0) {
				if (matchesGuid(position, res.getGuid())) {
					replaceValue(position, "height",
							new Integer(res.getHeight()).toString());
					replaceValue(position, "width",
							new Integer(res.getWidth()).toString());
					replaceValue(position, "hash",
							Global.byteArrayToHexString(res.getData()
									.getBodyHash()));
					return;
				}
			}
