...
public class ExportData {
	...
	public ExportData(DatabaseConnection conn2, boolean full, List<String> guids) {
		...
		for (int i = 0; i < guids.size(); i++) {
			notes.add(conn.getNoteTable().getNote(guids.get(i), true, true,
					true, true, true));
		}
		dirtyNotebooks = new HashMap<String, String>();
		localNotebooks = new HashMap<String, String>();
	...
	public void exportData(String filename) {
		...
		if (!xmlFile.open(QIODevice.OpenModeFlag.WriteOnly,
		...
		if (fullBackup) {
			...
			createTextNode("LastSequenceDate",
					new Long(sequenceDate).toString());
			writer.writeEndElement();
		}

		for (int i = 0; i < notes.size(); i++) {
			String guid = notes.get(i).getGuid();
			logger.log(logger.EXTREME, "Getting note " + guid + " : "
					+ notes.get(i).getTitle());
			Note note = conn.getNoteTable().getNote(guid, true, true, true,
					true, true);
	...
	private void writeSavedSearches() {
		...
		for (int i = 0; i < searches.size(); i++) {
			...
			createTextNode("Format", new Integer(searches.get(i).getFormat()
					.getValue()).toString());
	...
	private void writeLinkedNotebooks() {
		...
		for (int i = 0; i < linkedNotebooks.size(); i++) {
			...
			createTextNode("UpdateSequenceNumber", new Long(linkedNotebooks
					.get(i).getUpdateSequenceNum()).toString());
			if (dirtyLinkedNotebooks.containsKey(linkedNotebooks.get(i)
					.getGuid()))
				createTextNode("Dirty", "true");
			else
				createTextNode("Dirty", "false");
	...
	private void writeSharedNotebooks() {
		...
		for (int i = 0; i < linkedNotebooks.size(); i++) {
			...
			createTextNode("Id",
					new Long(sharedNotebooks.get(i).getId()).toString());
			createTextNode("Userid", new Integer(sharedNotebooks.get(i)
					.getUserId()).toString());
			createTextNode("Email", sharedNotebooks.get(i).getEmail());
			createTextNode("NotebookGuid", sharedNotebooks.get(i)
					.getNotebookGuid());
			createTextNode("ShareKey", sharedNotebooks.get(i).getShareKey());
			createTextNode("Username", sharedNotebooks.get(i).getUsername());
			createTextNode("ServiceCreated", new Long(sharedNotebooks.get(i)
					.getServiceCreated()).toString());
			if (dirtySharedNotebooks
					.containsKey(sharedNotebooks.get(i).getId()))
				createTextNode("Dirty", "true");
			else
				createTextNode("Dirty", "false");
			writer.writeEndElement();
	...
	private void writeNote(Note note) {
		...
		createTextNode("UpdateSequenceNumber",
				new Long(note.getUpdateSequenceNum()).toString());
		...
		if (noteAttributes != null) {
			...
			createTextNode("SourceApplication",
					noteAttributes.getSourceApplication());
			createTextNode("SourceURL", noteAttributes.getSourceURL());
			createTextNode("Altitude",
					new Double(noteAttributes.getAltitude()).toString());
			createTextNode("Longitude",
					new Double(noteAttributes.getLongitude()).toString());
			createTextNode("Latitude",
					new Double(noteAttributes.getLatitude()).toString());
			createTextNode("SubjectDate",
					new Long(noteAttributes.getSubjectDate()).toString());
			writer.writeEndElement();
		}

		// writeResources(conn.getNoteTable().noteResourceTable.getNoteResources(note.getGuid(),
		// true));
		writeResources(note.getResources());

		logger.log(logger.EXTREME, "Writing content");
		writer.writeStartElement("Content");
		writer.writeCDATA(conn.getNoteTable().getNoteContentNoUTFConversion(
				note.getGuid()));
		writer.writeEndElement();
		writer.writeEndElement();
	}
	...
	private void writeResources(List<Resource> resourceTable) {
		...
		for (int i = 0; i < resourceTable.size(); i++) {
			...
			createTextNode("UpdateSequenceNumber",
					new Integer(resource.getUpdateSequenceNum()).toString());
			createTextNode("Mime", resource.getMime());
			createTextNode("Duration",
					new Integer(resource.getDuration()).toString());
			createTextNode("Height",
					new Integer(resource.getHeight()).toString());
			...
			if (resource.getAttributes() != null) {
				...
				createTextNode("CameraMake", resource.getAttributes()
						.getCameraMake());
				createTextNode("CameraModel", resource.getAttributes()
						.getCameraModel());
				createTextNode("FileName", resource.getAttributes()
						.getFileName());
				createTextNode("RecoType", resource.getAttributes()
						.getRecoType());
				createTextNode("SourceURL", resource.getAttributes()
						.getSourceURL());
				createTextNode("Altitude", new Double(resource.getAttributes()
						.getAltitude()).toString());
				createTextNode("Longitude", new Double(resource.getAttributes()
						.getLongitude()).toString());
				createTextNode("Latitude", new Double(resource.getAttributes()
						.getLatitude()).toString());
				createTextNode("Timestamp", new Long(resource.getAttributes()
	...
	private void writeDataNode(String name, Data data) {
		...
		if (data.getBody() != null && data.getBody().length > 0)
			createBinaryNode("Body", new QByteArray(data.getBody()).toHex()
					.toString());
		else
			createBinaryNode("Body", "");
		if (data.getBodyHash() != null && data.getBodyHash().length > 0)
			createTextNode("BodyHash", new QByteArray(data.getBodyHash())
					.toHex().toString());
		else
	...
	private void writeRecognitionNode(String name, Data data) {
		...
		if (data.getBody() != null && data.getBody().length > 0) {
			...
			// writer.writeCDATA(new
			// QByteArray(data.getBody()).toHex().toString());
			writer.writeEndElement();
		} else
			createBinaryNode("Body", "");
	...
	private void writeNotebooks() {
		for (int i = 0; i < notebooks.size(); i++) {
			if (exportableNotebooks.containsKey(notebooks.get(i).getGuid())
					|| fullBackup) {
				writer.writeStartElement("Notebook");
				createTextNode("Guid", notebooks.get(i).getGuid());
				createTextNode("Name", notebooks.get(i).getName());
				createTextNode("UpdateSequenceNumber", new Long(notebooks
						.get(i).getUpdateSequenceNum()).toString());
				if (notebooks.get(i).isDefaultNotebook())
					createTextNode("DefaultNotebook", "true");
				else
					createTextNode("DefaultNotebook", "false");
				createTextNode("ServiceCreated", new Long(notebooks.get(i)
						.getServiceCreated()).toString());
				createTextNode("ServiceUpdated", new Long(notebooks.get(i)
						.getServiceUpdated()).toString());
				...
				if (dirtyNotebooks.containsKey(notebooks.get(i).getGuid()))
					...
					createTextNode("Dirty", "false");
				if (conn.getNotebookTable().isReadOnly(
				...
				if (notebooks.get(i).getPublishing() != null) {
					...
					createTextNode("PublishingPublicDescription",
							p.getPublicDescription());
					createTextNode("PublishingUri", p.getUri());
					createTextNode("PublishingOrder", new Integer(p.getOrder()
							.getValue()).toString());
					if (p.isAscending())
				...
				QByteArray b = conn.getNotebookTable().getIconAsByteArray(
						notebooks.get(i).getGuid());
				if (b != null)
					createBinaryNode("Icon", b.toHex().toString());
				if (notebooks.get(i).getStack() != null
						&& !notebooks.get(i).getStack().trim().equals(""))
	...
	private void writeTags() {
		for (int i = 0; i < tags.size(); i++) {
			if (exportableTags.containsKey(tags.get(i).getGuid()) || fullBackup) {
				...
				createTextNode("UpdateSequenceNumber", new Long(tags.get(i)
						.getUpdateSequenceNum()).toString());
	...
	private void createBinaryNode(String nodeName, String value) {
		...
		/*
		 * int i=0; for (; i<value.length(); i+=80) {
