...
public class CounterRunner extends QObject implements Runnable {
	...
	public CounterRunner(String logname, int t, String u, String i, String r,
			...
			for (int i = 0; i < idx.size(); i++) {
				...
				for (int j = 0; j < idx.get(i).getTagGuidsSize(); j++) {
					if (abortCount)
						return;
					record.tags
							.add(new String(idx.get(i).getTagGuids().get(j)));
				}
				records.add(record);
			}
		}
		...
		for (int i = 0; i < records.size(); i++) {
			...
			for (int j = 0; j < nCounter.size(); j++) {
				...
				if (records.get(i).active
						&& nCounter.get(j).getGuid()
								.equals(records.get(i).notebookGuid)) {
	...
	/*
	 * private void countTagResults() { logger.log(logger.EXTREME,
	 * "Entering ListManager.countTagResults"); List<TagCounter> counter = new
	 * ArrayList<TagCounter>(); List<Tag> allTags = conn.getTagTable().getAll();
	 * 
	 * if (abortCount) return; if (allTags == null) return; for (int k=0;
	 * k<allTags.size() && keepRunning; k++) { TagCounter newCounter = new
	 * TagCounter(); newCounter.setGuid(allTags.get(k).getGuid());
	 * newCounter.setCount(0); counter.add(newCounter); }
	 * 
	 * if (type == TAG_ALL) { List<Pair<String, Integer>> tagCounts =
	 * conn.getNoteTable().noteTagsTable.getTagCounts(); if (abortCount) return;
	 * for (int i=0; tagCounts != null && i<tagCounts.size(); i++) { if
	 * (abortCount) return; for (int j=0; j<counter.size(); j++) { if
	 * (abortCount) return; if
	 * (tagCounts.get(i).getFirst().equals(counter.get(j).getGuid())) { if
	 * (abortCount) return;
	 * counter.get(j).setCount(tagCounts.get(i).getSecond()); j=counter.size();
	 * } } } if (abortCount) return; tagSignal.countsChanged.emit(counter);
	 * return; }
	 * 
	 * 
	 * if (abortCount) return; List<cx.fbn.nevernote.sql.NoteTagsRecord> tags =
	 * conn.getNoteTable().noteTagsTable.getAllNoteTags(); for (int
	 * i=noteIndex.size()-1; i>=0; i--) { if (abortCount) return; String note =
	 * noteIndex.get(i); for (int x=0; tags!= null && x<tags.size() &&
	 * keepRunning; x++) { if (abortCount) return; String tag =
	 * tags.get(x).tagGuid; for (int j=0; j<counter.size() && keepRunning; j++)
	 * { if (abortCount) return; if (counter.get(j).getGuid().equals(tag) &&
	 * note.equals(tags.get(x).noteGuid)) { int c = counter.get(j).getCount()+1;
	 * counter.get(j).setCount(c); } } } } if (abortCount) return;
	 * tagSignal.countsChanged.emit(counter); logger.log(logger.EXTREME,
	 * "Leaving ListManager.countTagResults()"); }
	 */
			...
			for (int z = 0; z < tags.size(); z++) {
				...
				for (int j = 0; j < tCounter.size(); j++) {
					...
					if (tCounter.get(j).getGuid().equals(tags.get(z))) {
						tCounter.get(j)
								.setCount(tCounter.get(j).getCount() + 1);
						found = true;
						j = tCounter.size();
					}
				}
