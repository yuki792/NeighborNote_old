...
public class SyncRunner extends QObject implements Runnable {
	...
	public SyncRunner(String logname, String u, String i, String r, String b,
	...
	public void run() {
		...
		try {
			...
			conn = new DatabaseConnection(logger, dburl, indexUrl, resourceUrl,
					behaviorUrl, dbuid, dbpswd, dbcpswd, 200);

			while (keepRunning) {
				logger.log(logger.EXTREME, "Blocking until work is found");
				String work = workQueue.take();
				logger.log(logger.LOW,
						"Dirty Notes Before Sync: "
								+ new Integer(conn.getNoteTable()
										.getDirtyCount()).toString());
				...
				if (syncNeeded) {
					...
					updateSequenceNumber = conn.getSyncTable()
				...
				logger.log(logger.EXTREME,
						"Signaling refresh finished.  refreshNeeded="
								+ refreshNeeded);
				syncSignal.finished.emit(refreshNeeded);
				if (error) {
					syncSignal.errorDisconnect.emit();
					status.message
							.emit(tr("Error synchronizing - see log for details."));
				}
				logger.log(logger.LOW,
						"Dirty Notes After Sync: "
								+ new Integer(conn.getNoteTable()
										.getDirtyCount()).toString());
			}
	...
	private void evernoteSync(Client noteStore)
		...
		if (isConnected && keepRunning) {
			...
			try {
				...
				e1.printStackTrace();
				status.message
						.emit(tr("User exception getting user account information.  Aborting sync and disconnecting"));
				syncSignal.errorDisconnect.emit();
				error = true;
				enDisconnect();
				return;
				...
				e1.printStackTrace();
				status.message
						.emit(tr("System error user account information.  Aborting sync and disconnecting!"));
				syncSignal.errorDisconnect.emit();
				error = true;
				enDisconnect();
				return;
				...
				error = true;
				status.message
						.emit(tr("Transaction error getting user account information.  Aborting sync and disconnecting!"));
				enDisconnect();
				return;
			}
			...
			try {
				...
				syncSignal.saveEvernoteUpdateCount.emit(syncState
						.getUpdateCount());
				evernoteUpdateCount = syncState.getUpdateCount();
			} catch (EDAMUserException e) {
				e.printStackTrace();
				status.message
						.emit(tr("Error getting sync state! Aborting sync and disconnecting!"));
				syncSignal.errorDisconnect.emit();
				enDisconnect();
				return;
			...
			logger.log(logger.LOW,
					"Full Sequence Before: " + syncState.getFullSyncBefore());
			logger.log(logger.LOW, "Last Sequence Date: " + sequenceDate);
			logger.log(logger.LOW, "Var Last Sequence Number: "
					+ updateSequenceNumber);
			logger.log(logger.LOW, "DB Last Sequence Number: "
					+ conn.getSyncTable().getUpdateSequenceNumber());
			...
			String syncLinked = conn.getSyncTable().getRecord(
					"FullLinkedNotebookSync");
			String syncShared = conn.getSyncTable().getRecord(
					"FullSharedNotebookSync");
			String syncNotebooks = conn.getSyncTable().getRecord(
					"FullNotebookSync");
			String syncInkNoteImages = conn.getSyncTable().getRecord(
					"FullInkNoteImageSync");
			if (syncLinked != null) {
			...
			if (syncInkNoteImages != null) {
				List<String> guids = conn.getNoteTable().noteResourceTable
						.findInkNotes();
			...
			logger.log(logger.LOW,
					"Update Count: " + syncState.getUpdateCount());
			...
			List<String> notes = conn.getNoteTable()
					.expungeIgnoreSynchronizedNotes(
							conn.getSyncTable().getIgnoreRecords("NOTEBOOK"),
							conn.getSyncTable().getIgnoreRecords("TAG"),
							conn.getSyncTable().getIgnoreRecords(
									"LINKEDNOTEBOOK"));
			if (notes.size() > 0)
			...
			if (!error) {
				...
				if (!disableUploads)
					...
					status.message
							.emit(tr("Download syncronization complete.  Uploads have been disabled."));
				...
				conn.getSyncTable().setUpdateSequenceNumber(
						updateSequenceNumber);
			}
		}
	...
	private void syncExpunged(Client noteStore) {
		...
		List<DeletedItemRecord> expunged = conn.getDeletedTable()
				.getAllDeleted();
		boolean error = false;
		for (int i = 0; i < expunged.size() && keepRunning; i++) {
			...
			try {
				if (expunged.get(i).type.equalsIgnoreCase("TAG")) {
					...
					updateSequenceNumber = noteStore.expungeTag(authToken,
							expunged.get(i).guid);
					conn.getSyncTable().setUpdateSequenceNumber(
							updateSequenceNumber);
					conn.getSyncTable().setLastSequenceDate(sequenceDate);
					conn.getSyncTable().setUpdateSequenceNumber(
							updateSequenceNumber);
					conn.getDeletedTable().expungeDeletedItem(
							expunged.get(i).guid, "TAG");
				}
				if (expunged.get(i).type.equalsIgnoreCase("NOTEBOOK")) {
					logger.log(logger.EXTREME, "Notebook expunged");
					updateSequenceNumber = noteStore.expungeNotebook(authToken,
							expunged.get(i).guid);
					conn.getSyncTable().setLastSequenceDate(sequenceDate);
					conn.getSyncTable().setUpdateSequenceNumber(
							updateSequenceNumber);
					conn.getDeletedTable().expungeDeletedItem(
							expunged.get(i).guid, "NOTEBOOK");
				}
				if (expunged.get(i).type.equalsIgnoreCase("NOTE")) {
					logger.log(logger.EXTREME, "Note expunged");
					updateSequenceNumber = noteStore.deleteNote(authToken,
							expunged.get(i).guid);
					refreshNeeded = true;
					conn.getSyncTable().setLastSequenceDate(sequenceDate);
					conn.getSyncTable().setUpdateSequenceNumber(
							updateSequenceNumber);
					conn.getDeletedTable().expungeDeletedItem(
							expunged.get(i).guid, "NOTE");
				}
				if (expunged.get(i).type.equalsIgnoreCase("SAVEDSEARCH")) {
					logger.log(logger.EXTREME, "saved search expunged");
					updateSequenceNumber = noteStore.expungeSearch(authToken,
							expunged.get(i).guid);
					conn.getSyncTable().setLastSequenceDate(sequenceDate);
					conn.getSyncTable().setUpdateSequenceNumber(
							updateSequenceNumber);
					conn.getDeletedTable().expungeDeletedItem(
							expunged.get(i).guid, "SAVEDSEARCH");
				}
			} catch (EDAMUserException e) {
				logger.log(logger.LOW, "EDAM User Excepton in syncExpunged: "
						+ expunged.get(i).guid); // This can happen if we try to
													// delete a deleted note
			} catch (EDAMSystemException e) {
				logger.log(logger.LOW, "EDAM System Excepton in syncExpunged: "
						+ expunged.get(i).guid);
				...
				logger.log(
						logger.LOW,
						"EDAM Not Found Excepton in syncExpunged: "
								+ expunged.get(i).guid);
			} catch (TException e) {
				logger.log(logger.LOW, "EDAM TExcepton in syncExpunged: "
						+ expunged.get(i).guid);
				logger.log(logger.LOW, e.getStackTrace());
				error = true;
			}
		...
		for (int i = 0; i < notes.size() && keepRunning; i++) {
			...
			try {
				if (enNote.getUpdateSequenceNum() > 0
						&& (enNote.isActive() == false || enNote.getDeleted() > 0)) {
					if (syncDeletedContent) {
						logger.log(logger.EXTREME,
								"Deleted note found & synch content selected");
						Note delNote = conn.getNoteTable().getNote(
								enNote.getGuid(), true, true, true, true, true);
						delNote = getNoteContent(delNote);
						delNote = noteStore.updateNote(authToken, delNote);
						enNote.setUpdateSequenceNum(delNote
								.getUpdateSequenceNum());
						conn.getNoteTable()
								.updateNoteSequence(enNote.getGuid(),
										enNote.getUpdateSequenceNum());
					} else {
						logger.log(logger.EXTREME,
								"Deleted note found & sync content not selected");
						int usn = noteStore.deleteNote(authToken,
								enNote.getGuid());
				...
				// logger.log(logger.LOW,
				// "*** EDAM User Excepton syncLocalNotes "+e);
				// status.message.emit("Error sending local note: "
				// +e.getParameter());
				// logger.log(logger.LOW, e.toString());
				// error = true;
			} catch (EDAMSystemException e) {
				logger.log(logger.LOW,
						"*** EDAM System Excepton syncLocalNotes " + e);
				status.message.emit(tr("Error: ") + e);
				logger.log(logger.LOW, e.toString());
				error = true;
			} catch (EDAMNotFoundException e) {
				// logger.log(logger.LOW,
				// "*** EDAM Not Found Excepton syncLocalNotes " +e);
				// status.message.emit("Error deleting local note: " +e
				// +" - Continuing");
				// logger.log(logger.LOW, e.toString());
				// error = true;
			} catch (TException e) {
	...
	private void syncLocalNotes() {
		...
		logger.log(logger.LOW, "Dirty local notes found: "
				+ new Integer(conn.getNoteTable().getDirtyCount()).toString());
		...
		if (enNote.isActive()) {
			try {
				if (enNote.getUpdateSequenceNum() > 0) {
					...
					enNote = getNoteContent(enNote);
					logger.log(logger.MEDIUM,
							"Creating note : " + enNote.getGuid() + " <title>"
									+ enNote.getTitle() + "</title>");
					enNote = noteStore.createNote(token, enNote);
					logger.log(logger.MEDIUM,
							"New note Guid : " + enNote.getGuid() + " <title>"
									+ enNote.getTitle() + "</title>");
					noteSignal.guidChanged.emit(oldGuid, enNote.getGuid());
					conn.getNoteTable().updateNoteGuid(oldGuid,
							enNote.getGuid());
				}
				updateSequenceNumber = enNote.getUpdateSequenceNum();
				logger.log(logger.EXTREME, "Saving note");
				conn.getNoteTable().updateNoteSequence(enNote.getGuid(),
						enNote.getUpdateSequenceNum());
				...
				for (int j = 0; j < enNote.getResourcesSize() && keepRunning; j++) {
					...
					if (d != null) {
						...
						String oldGuid = conn.getNoteTable().noteResourceTable
								.getNoteResourceGuidByHashHex(enNote.getGuid(),
										hash);
						conn.getNoteTable().updateNoteResourceGuidbyHash(
								enNote.getGuid(), newRes.getGuid(), hash);
						resourceSignal.resourceGuidChanged.emit(
								enNote.getGuid(), oldGuid, newRes.getGuid());
					}
				}
				...
				updateSequenceNumber = enNote.getUpdateSequenceNum();
				logger.log(logger.EXTREME,
						"Emitting note sequence number change");
				conn.getSyncTable().setUpdateSequenceNumber(
						updateSequenceNumber);

			} catch (EDAMUserException e) {
				logger.log(logger.LOW, "*** EDAM User Excepton syncLocalNotes "
						+ e);
				status.message.emit(tr("Error sending local note: ")
						+ e.getParameter());
				logger.log(logger.LOW, e.toString());
				error = true;
			} catch (EDAMSystemException e) {
	...
	private void syncLocalNotebooks(Client noteStore) {
		...
		try {
			logger.log(logger.EXTREME,
					"Getting remote notebooks to compare with local");
			remoteList = noteStore.listNotebooks(authToken);
		} catch (EDAMUserException e1) {
			logger.log(logger.LOW,
					"*** EDAM User Excepton syncLocalNotebooks getting remote Notebook List");
			status.message.emit(tr("Error: ") + e1);
			logger.log(logger.LOW, e1.toString());
			error = true;
		} catch (EDAMSystemException e1) {
			logger.log(logger.LOW,
					"*** EDAM System Excepton syncLocalNotebooks getting remote Notebook List");
			status.message.emit(tr("Error: ") + e1);
			logger.log(logger.LOW, e1.toString());
			error = true;
		} catch (TException e1) {
			logger.log(logger.LOW,
					"*** EDAM Transaction Excepton syncLocalNotebooks getting remote Notebook List");
			status.message.emit(tr("Error: ") + e1);
			logger.log(logger.LOW, e1.toString());
			error = true;
		}
		...
		for (int i = 0; i < notebooks.size() && keepRunning; i++) {
			...
			try {
				if (enNotebook.getUpdateSequenceNum() > 0) {
					...
					// Look for a notebook with the same name. If one is found,
					// we don't need
					// to create another one
					logger.log(logger.EXTREME,
							"Looking for matching notebook name");
					for (int k = 0; k < remoteList.size() && !found
							&& keepRunning; k++) {
						if (remoteList.get(k).getName()
								.equalsIgnoreCase(enNotebook.getName())) {
							enNotebook = remoteList.get(k);
							logger.log(logger.EXTREME,
									"Matching notebook found");
							found = true;
						}
					}
					if (!found)
						enNotebook = noteStore.createNotebook(authToken,
								enNotebook);

					logger.log(logger.EXTREME, "Updating notebook in database");
					conn.getNotebookTable().updateNotebookGuid(oldGuid,
							enNotebook.getGuid());
					sequence = enNotebook.getUpdateSequenceNum();
				}
				logger.log(logger.EXTREME,
						"Updating notebook sequence in database");
				conn.getNotebookTable().updateNotebookSequence(
						enNotebook.getGuid(), sequence);
				...
				logger.log(logger.EXTREME,
						"Emitting sequence number to main thread");
				conn.getSyncTable().setUpdateSequenceNumber(
						updateSequenceNumber);
			} catch (EDAMUserException e) {
				logger.log(logger.LOW,
						"*** EDAM User Excepton syncLocalNotebooks");
				logger.log(logger.LOW, e.toString());
				error = true;
			} catch (EDAMSystemException e) {
				logger.log(logger.LOW,
						"*** EDAM System Excepton syncLocalNotebooks");
				logger.log(logger.LOW, e.toString());
				error = true;
			} catch (EDAMNotFoundException e) {
				logger.log(logger.LOW,
						"*** EDAM Not Found Excepton syncLocalNotebooks");
				logger.log(logger.LOW, e.toString());
				error = true;
			} catch (TException e) {
	...
	private void syncLocalTags(Client noteStore) {
		...
		try {
			logger.log(logger.EXTREME,
					"Getting remote tags to compare names with the local tags");
			remoteList = noteStore.listTags(authToken);
		} catch (EDAMUserException e1) {
			logger.log(logger.LOW,
					"*** EDAM User Excepton syncLocalTags getting remote Tag List");
			status.message.emit(tr("Error: ") + e1);
			logger.log(logger.LOW, e1.toString());
			error = true;
		} catch (EDAMSystemException e1) {
			logger.log(logger.LOW,
					"*** EDAM System Excepton syncLocalTags getting remote Tag List");
			status.message.emit(tr("Error: ") + e1);
			logger.log(logger.LOW, e1.toString());
			error = true;
		} catch (TException e1) {
			logger.log(logger.LOW,
					"*** EDAM Transaction Excepton syncLocalTags getting remote Tag List");
			status.message.emit(tr("Error: ") + e1);
			logger.log(logger.LOW, e1.toString());
			error = true;
		}
		...
		while (enTag != null && loopCount < maxCount) {
			...
			try {
				if (enTag.getUpdateSequenceNum() > 0) {
					...
					// Look for a tag with the same name. If one is found, we
					// don't need
					// to create another one
					logger.log(logger.EXTREME,
							"New tag.  Comparing with remote names");
					boolean found = false;
					String oldGuid = enTag.getGuid();
					for (int k = 0; k < remoteList.size() && !found
							&& keepRunning; k++) {
						if (remoteList.get(k).getName()
								.equalsIgnoreCase(enTag.getName())) {
							conn.getTagTable().updateTagGuid(enTag.getGuid(),
									remoteList.get(k).getGuid());
							enTag = remoteList.get(k);
							logger.log(logger.EXTREME,
									"Matching tag name found");
							found = true;
						}
					}
					if (!found)
				...
				logger.log(logger.EXTREME,
						"Emitting sequence number to the main thread.");
				updateSequenceNumber = sequence;
				conn.getSyncTable().setUpdateSequenceNumber(
						updateSequenceNumber);
			} catch (EDAMUserException e) {
				...
				logger.log(
						logger.LOW,
						"** EDAM System Excepton syncLocalTags: "
								+ enTag.getName());
				logger.log(logger.LOW, e.toString());
				badTagSync.put(enTag.getGuid(), null);
				error = true;
				...
				logger.log(
						logger.LOW,
						"*** EDAM Not Found Excepton syncLocalTags: "
								+ enTag.getName());
				logger.log(logger.LOW, e.toString());
				badTagSync.put(enTag.getGuid(), null);
				error = true;
	...
	private void syncLocalLinkedNotebooks(Client noteStore) {
		...
		for (int i = 0; i < list.size(); i++) {
			LinkedNotebook book = conn.getLinkedNotebookTable().getNotebook(
					list.get(i));
			try {
				noteStore.updateLinkedNotebook(authToken, book);
			} catch (EDAMUserException e) {
				logger.log(logger.LOW,
						"*** EDAM User Excepton syncLocalLinkedNotebooks");
				status.message.emit(tr("Error: ") + e);
				logger.log(logger.LOW, e.toString());
				error = true;
				e.printStackTrace();
				...
				logger.log(logger.LOW,
						"*** EDAM Not Found Excepton syncLocalLinkedNotebooks");
				status.message.emit(tr("Error: ") + e);
				logger.log(logger.LOW, e.toString());
				error = true;
				e.printStackTrace();
				...
				logger.log(logger.LOW,
						"*** EDAM System Excepton syncLocalLinkedNotebooks");
				status.message.emit(tr("Error: ") + e);
				logger.log(logger.LOW, e.toString());
				error = true;
				e.printStackTrace();
				...
				logger.log(logger.LOW,
						"*** EDAM TExcepton syncLocalLinkedNotebooks");
				status.message.emit(tr("Error: ") + e);
				logger.log(logger.LOW, e.toString());
				error = true;
				e.printStackTrace();
	...
	private void syncLocalSavedSearches(Client noteStore) {
		...
		logger.log(logger.EXTREME,
				"Getting saved searches to compare with local");
		try {
			remoteList = noteStore.listSearches(authToken);
		} catch (EDAMUserException e1) {
			logger.log(logger.LOW,
					"*** EDAM User Excepton syncLocalTags getting remote saved search List");
			status.message.emit(tr("Error: ") + e1);
			logger.log(logger.LOW, e1.toString());
			error = true;
		} catch (EDAMSystemException e1) {
			logger.log(logger.LOW,
					"*** EDAM System Excepton syncLocalTags getting remote saved search List");
			status.message.emit(tr("Error: ") + e1);
			logger.log(logger.LOW, e1.toString());
			error = true;
		} catch (TException e1) {
			logger.log(logger.LOW,
					"*** EDAM Transaction Excepton syncLocalTags getting remote saved search List");
			status.message.emit(tr("Error: ") + e1);
			logger.log(logger.LOW, e1.toString());
			error = true;
		}
		...
		for (int i = 0; i < searches.size() && keepRunning; i++) {
			...
			try {
				if (enSearch.getUpdateSequenceNum() > 0)
					...
					boolean found = false;
					logger.log(logger.EXTREME,
							"Matching remote saved search names with local");
					for (int k = 0; k < remoteList.size() && !found
							&& keepRunning; k++) {
						if (remoteList.get(k).getName()
								.equalsIgnoreCase(enSearch.getName())) {
							enSearch = remoteList.get(k);
							found = true;
							logger.log(logger.EXTREME,
									"Matching saved search found");
					...
					sequence = enSearch.getUpdateSequenceNum();
					logger.log(logger.EXTREME,
							"Updating tag guid in local database");
					conn.getSavedSearchTable().updateSavedSearchGuid(oldGuid,
							enSearch.getGuid());
				}
				logger.log(logger.EXTREME,
						"Updating tag sequence in local database");
				conn.getSavedSearchTable().updateSavedSearchSequence(
						enSearch.getGuid(), sequence);
				logger.log(logger.EXTREME, "Resetting tag dirty flag");
	...
	private void syncRemoteToLocal(Client noteStore) {
		...
		while (more && keepRunning) {
			...
			try {
				...
				chunk = noteStore.getSyncChunk(authToken, sequence, chunkSize,
						fullSync);
				logger.log(logger.LOW,
						"Chunk High Sequence: " + chunk.getChunkHighUSN());
			} catch (EDAMUserException e) {
				error = true;
				e.printStackTrace();
				status.message.emit(e.getMessage());
			...
			for (int i = 0; i < chunk.getNotesSize(); i++)
				noteSignal.noteChanged.emit(chunk.getNotes().get(i).getGuid(),
						null);
			...
			if (more) {
				...
				status.message.emit(tr("Downloading ")
						+ new Long(pct).toString() + tr("% complete."));
			}
			// conn.commitTransaction();
		}
	...
	private void syncExpungedNotes(SyncChunk chunk) {
		...
		if (guid != null) {
			for (int i = 0; i < guid.size() && keepRunning; i++) {
				String notebookGuid = "";
				Note localNote = conn.getNoteTable().getNote(guid.get(i),
				...
				if (!conn.getNotebookTable().isNotebookLocal(notebookGuid)) {
					logger.log(logger.EXTREME,
							"Expunging local note from database");
		...
		if (guid != null)
			for (int i = 0; i < guid.size() && keepRunning; i++) {
				logger.log(logger.EXTREME,
						"Expunging local notebook from database");
		...
		if (guid != null)
			for (int i = 0; i < guid.size() && keepRunning; i++) {
				logger.log(logger.EXTREME,
						"Expunging saved search from local database");
		...
		if (guid != null)
			for (int i = 0; i < guid.size() && keepRunning; i++) {
				logger.log(logger.EXTREME,
						"Expunging linked notebook from local database");
				conn.getLinkedNotebookTable().expungeNotebook(guid.get(i),
						false);
			}
	...
	private void syncRemoteTags(List<Tag> tags) {
		...
		if (tags != null) {
			for (int i = 0; i < tags.size() && keepRunning; i++) {
				String oldGuid;
				oldGuid = conn.getTagTable().findTagByName(
						tags.get(i).getName());
				if (oldGuid != null
						&& !tags.get(i).getGuid().equalsIgnoreCase(oldGuid))
					conn.getTagTable().updateTagGuid(oldGuid,
							tags.get(i).getGuid());
	...
	private void syncRemoteSavedSearches(List<SavedSearch> searches) {
		...
		if (searches != null) {
			for (int i = 0; i < searches.size() && keepRunning; i++) {
				String oldGuid;
				oldGuid = conn.getSavedSearchTable().findSavedSearchByName(
						searches.get(i).getName());
				if (oldGuid != null
						&& !searches.get(i).getGuid().equalsIgnoreCase(oldGuid))
					conn.getSavedSearchTable().updateSavedSearchGuid(oldGuid,
							searches.get(i).getGuid());
				conn.getSavedSearchTable().syncSavedSearch(searches.get(i),
						false);
			}
		}
	...
	private void syncRemoteLinkedNotebooks(List<LinkedNotebook> books) {
		...
		if (books != null) {
			for (int i = 0; i < books.size() && keepRunning; i++) {
				conn.getLinkedNotebookTable().updateNotebook(books.get(i),
						false);
			}
		}
	...
	private void syncRemoteNotebooks(List<Notebook> notebooks) {
		...
		if (notebooks != null) {
			for (int i = 0; i < notebooks.size() && keepRunning; i++) {
				String oldGuid;
				oldGuid = conn.getNotebookTable().findNotebookByName(
						notebooks.get(i).getName());
				if (oldGuid != null
						&& !conn.getNotebookTable().isNotebookLocal(oldGuid)
						&& !notebooks.get(i).getGuid()
								.equalsIgnoreCase(oldGuid))
					conn.getNotebookTable().updateNotebookGuid(oldGuid,
							notebooks.get(i).getGuid());
				...
				// conn.getSharedNotebookTable().expungeNotebookByGuid(notebooks.get(i).getGuid(),
				// false);
				// for (int j=0; j<notebooks.get(i).getSharedNotebookIdsSize();
				// j++) {
				// syncRemoteSharedNotebook(notebooks.get(i).getGuid(),
				// notebooks.get(i).getSharedNotebookIds().get(j), authToken);
				// }
				// }
			}
		}
	...
	// {
	// List<SharedNotebook> books =
	// noteStore.getSharedNotebookByAuth(authToken);
	// }
	...
	private void syncRemoteResource(Client noteStore, Resource resource,
			String authToken) {
		// This is how the logic for this works.
		// 1.) If the resource is not in the local database, we add it.
		// 2.) If a copy of the resource is in the local database and the note
		// isn't dirty, we update the local copy
		// 3.) If a copy of the resource is in the local databbase and it is
		// dirty and the hash doesn't match, we ignore it because there
		// is a conflict. The note conflict should get a copy of the resource at
		// that time.

		Note n = conn.getNoteTable().getNote(resource.getNoteGuid(), false,
				false, false, false, false);
		if (n != null) {
			logger.log(logger.HIGH, "Resource for note " + n.getGuid() + " : "
					+ n.getTitle());
		}
		boolean saveNeeded = false;
		/* #1 */Resource r = getEvernoteResource(noteStore, resource.getGuid(),
				true, true, true, authToken);
		Resource l = conn.getNoteTable().noteResourceTable.getNoteResource(
				r.getGuid(), false);
		if (l == null) {
			logger.log(logger.HIGH, "Local resource not found");
			saveNeeded = true;
		} else {
			/* #2 */boolean isNoteDirty = conn.getNoteTable().isNoteDirty(
					r.getNoteGuid());
			if (!isNoteDirty) {
				logger.log(logger.HIGH,
						"Local resource found, but is not dirty");
				saveNeeded = true;
			} else {
				/* #3 */String remoteHash = "";
				if (r != null && r.getData() != null
						&& r.getData().getBodyHash() != null)
					remoteHash = byteArrayToHexString(r.getData().getBodyHash());
				String localHash = "";
				if (l != null && l.getData() != null
						&& l.getData().getBodyHash() != null)
	...
	// Sync remote notes
	private void syncRemoteNotes(Client noteStore, List<Note> note,
			boolean fullSync, String token) {
		logger.log(logger.EXTREME, "Entering SyncRunner.syncRemoteNotes");
		if (note != null) {
			for (int i = 0; i < note.size() && keepRunning; i++) {
				Note n = getEvernoteNote(noteStore, note.get(i).getGuid(),
						true, fullSync, true, true, token);
	...
	private void syncRemoteNote(Note n, boolean fullSync, String token) {
		if (n != null) {
			...
			// are different we move the local copy to a local notebook (making
			// sure
			// to copy all resources) and we accept the new one.
			boolean conflictingNote = true;
			logger.log(logger.EXTREME,
					"Checking for duplicate note " + n.getGuid());
			if (dirtyNoteGuids != null && dirtyNoteGuids.contains(n.getGuid())) {
				...
				logger.log(logger.EXTREME, "Conflict check results "
						+ conflictingNote);
				if (conflictingNote)
			...
			if ((conflictingNote || fullSync) && !ignoreNote) {
				...
				// The following was commented out because it caused a race
				// condition on the database where resources
				// may be lost. We do the same thing elsewhere;.
				// noteSignal.noteChanged.emit(n.getGuid(), null); // Signal to
				// ivalidate note cache
				noteSignal.noteDownloaded.emit(n, true); // Signal to add note
															// to index
				...
				if (fullSync && n.getResources() != null) {
					for (int q = 0; q < n.getResources().size() && keepRunning; q++) {
						...
						conn.getNoteTable().noteResourceTable
								.updateNoteResource(n.getResources().get(q),
										false);
						if (n.getResources()
								.get(q)
								.getMime()
							...
							downloadInkNoteImage(n.getResources().get(q)
									.getGuid(), token);
					}
				}
			}
	...
	private Note getEvernoteNote(Client noteStore, String guid,
			boolean withContent, boolean withResourceData,
			boolean withResourceRecognition, boolean withResourceAlternateData,
			String token) {
		Note n = null;
		try {
			logger.log(logger.EXTREME, "Retrieving note " + guid);
			n = noteStore.getNote(token, guid, withContent, withResourceData,
					withResourceRecognition, withResourceAlternateData);
			...
			logger.log(logger.LOW,
					"*** EDAM Not Found Excepton getEvernoteNote");
			logger.log(logger.LOW, e.toString());
			error = true;
			e.printStackTrace();
		} catch (TException e) {
	...
	private Resource getEvernoteResource(Client noteStore, String guid,
			boolean withData, boolean withRecognition, boolean withAttributes,
			String token) {
		Resource n = null;
		try {
			logger.log(logger.EXTREME, "Retrieving resource " + guid);
			n = noteStore.getResource(token, guid, withData, withRecognition,
					withAttributes, withAttributes);
			logger.log(logger.EXTREME, "Resource " + guid
					+ " has been retrieved.");
		} catch (EDAMUserException e) {
			logger.log(logger.LOW, "*** EDAM User Excepton getEvernoteNote");
			logger.log(logger.LOW, e.toString());
	...
	private boolean checkForConflict(Note n) {
		logger.log(logger.EXTREME,
				"Checking note sequence number  " + n.getGuid());
		Note oldNote = conn.getNoteTable().getNote(n.getGuid(), false, false,
				false, false, false);
		logger.log(
				logger.EXTREME,
				"Local/Remote sequence numbers: "
						+ oldNote.getUpdateSequenceNum() + "/"
	...
	private void moveConflictingNote(String guid) {
		...
		for (int i = 0; i < books.size() && keepRunning; i++) {
			if (books.get(i).getName()
					.equalsIgnoreCase("Conflicting Changes (local)")
					|| books.get(i).getName()
							.equalsIgnoreCase("Conflicting Changes")) {
		...
		if (notebookGuid == null) {
			logger.log(logger.EXTREME, "Building conflicting change notebook "
					+ guid);
		...
		Note oldNote = conn.getNoteTable().getNote(guid, true, true, false,
				false, false);
		for (int i = 0; i < oldNote.getResources().size() && keepRunning; i++) {
			...
			conn.getNoteTable().noteResourceTable.resetUpdateSequenceNumber(
					oldResG, true);
			conn.getNoteTable().noteResourceTable.updateNoteResourceGuid(
					oldResG, newResG, true);
		}
	...
	private Tag findNextTag() {
		...
		for (int i = 0; i < tags.size() && keepRunning; i++) {
			if (!badTagSync.containsKey(tags.get(i).getGuid())) {
				if (tags.get(i).getParentGuid() == null) {
					logger.log(logger.HIGH,
							"Leaving SyncRunner.findNextTag - tag found without parent");
					return tags.get(i);
				}
				Tag parentTag = conn.getTagTable().getTag(
						tags.get(i).getParentGuid());
				if (parentTag.getUpdateSequenceNum() > 0) {
					logger.log(logger.HIGH,
							"Leaving SyncRunner.findNextTag - tag found");
					return tags.get(i);
				}
			}
		}
		...
		logger.log(logger.HIGH,
				"Leaving SyncRunner.findNextTag - no tags returned");
		return nextTag;
	}
	...
	public boolean enConnect() {
		try {
			...
			QMessageBox mb = new QMessageBox(QMessageBox.Icon.Critical,
					"Transport Excepton", e.getLocalizedMessage());
			mb.exec();
			e.printStackTrace();
		}
		...
		try {
			...
			QMessageBox mb = new QMessageBox(QMessageBox.Icon.Critical,
					"EDAM System Excepton", e.getLocalizedMessage());
			mb.exec();
			e.printStackTrace();
			isConnected = false;
		} catch (TException e) {
		...
		isConnected = true;
		// authTimeRemaining = authResult.getExpiration() -
		// authResult.getCurrentTime();
	...
	/*
	 * // Refresh the connection private synchronized boolean
	 * refreshConnection() {
	 *
	 * logger.log(logger.EXTREME, "Entering SyncRunner.refreshConnection()"); //
	 * Calendar cal = Calendar.getInstance();
	 *
	 * // If we are not connected let's get out of here if (!isConnected) return
	 * false;
	 *
	 * // If we fail too many times, then let's give up. if (failedRefreshes
	 * >=5) { logger.log(logger.EXTREME,
	 * "Refresh attempts have failed.  Disconnecting."); isConnected = false;
	 * status.message.emit(tr("Unable to synchronize - Authentication failed"));
	 * return false; }
	 *
	 * // If this is the first time through, then we need to set this // if
	 * (authRefreshTime == 0 || cal.getTimeInMillis() > authRefreshTime) //
	 ...
	 *
	 * // Try to get a new token AuthenticationResult newAuth = null;
	 * logger.log(logger.EXTREME, "Beginning to try authentication refresh");
	 * try { if (userStore != null && authToken != null) newAuth =
	 * userStore.refreshAuthentication(authToken); else return false;
	 * logger.log(logger.EXTREME,
	 * "UserStore.refreshAuthentication has succeeded."); } catch
	 * (EDAMUserException e) { e.printStackTrace();
	 * syncSignal.authRefreshComplete.emit(false); failedRefreshes++; return
	 * false; } catch (EDAMSystemException e) { e.printStackTrace();
	 * syncSignal.authRefreshComplete.emit(false); failedRefreshes++; return
	 * false; } catch (TException e) { e.printStackTrace();
	 * syncSignal.authRefreshComplete.emit(false); failedRefreshes++; return
	 * false; }
	 *
	 * // If we didn't get a good auth, then we've failed if (newAuth == null) {
	 * failedRefreshes++;
	 * status.message.emit(tr("Unable to synchronize - Authentication failed"));
	 * logger.log(logger.EXTREME, "Authentication failure #" +failedRefreshes);
	 * status.message.emit(tr("Unable to synchronize - Authentication failed"));
	 * syncSignal.authRefreshComplete.emit(false); return false; }
	 *
	 * // We got a good token. Now we need to setup the time to renew it.
	 * logger.log(logger.EXTREME, "Saving authentication tokens"); authResult =
	 * newAuth; authToken = new String(newAuth.getAuthenticationToken()); //
	 * authTimeRemaining = authResult.getExpiration() -
	 * authResult.getCurrentTime(); // authRefreshTime = cal.getTimeInMillis() +
	 * (authTimeRemaining/4); failedRefreshes=0;
	 * syncSignal.authRefreshComplete.emit(true); authRefreshNeeded = false;
	 *
	 * // This should never happen, but if it does we consider this a faild
	 * attempt. // if (authTimeRemaining <= 0) { // failedRefreshes++; //
	...
	private void downloadAllSharedNotebooks(Client noteStore) {
		try {
			List<SharedNotebook> books = noteStore
					.listSharedNotebooks(authToken);
			logger.log(logger.LOW, "Shared notebooks found = " + books.size());
			for (int i = 0; i < books.size(); i++) {
				conn.getSharedNotebookTable().updateNotebook(books.get(i),
						false);
			}
			...
			e1.printStackTrace();
			status.message
					.emit(tr("System exception Listing shared notebooks."));
			logger.log(logger.LOW, e1.getMessage());
			return;
		} catch (TException e1) {
			e1.printStackTrace();
			status.message
					.emit(tr("Transaction exception Listing shared notebooks."));
			logger.log(logger.LOW, e1.getMessage());
			return;
		} catch (EDAMNotFoundException e1) {
			e1.printStackTrace();
			status.message
					.emit(tr("EDAM Not Found exception Listing shared notebooks."));
			logger.log(logger.LOW, e1.getMessage());
		}
	}
	...
	private void downloadAllLinkedNotebooks(Client noteStore) {
		try {
			List<LinkedNotebook> books = noteStore
			...
			e1.printStackTrace();
			status.message
					.emit(tr("System exception Listing linked notebooks."));
			logger.log(logger.LOW, e1.getMessage());
			return;
		} catch (TException e1) {
			e1.printStackTrace();
			status.message
					.emit(tr("Transaction exception Listing lineked notebooks."));
			logger.log(logger.LOW, e1.getMessage());
			return;
		} catch (EDAMNotFoundException e1) {
			e1.printStackTrace();
			status.message
					.emit(tr("EDAM Not Found exception Listing linked notebooks."));
			logger.log(logger.LOW, e1.getMessage());
		}
	}

	private void downloadInkNoteImage(String guid, String authToken) {
		String urlBase = noteStoreUrl.replace("/edam/note/", "/shard/")
				+ "/res/" + guid + ".ink?slice=";
		// urlBase =
		// "https://www.evernote.com/shard/s1/res/52b567a9-54ae-4a08-afc5-d5bae275b2a8.ink?slice=";
		Integer slice = 1;
		...
		noteSignal.noteChanged.emit(r.getNoteGuid(), null); // Signal to
															// ivalidate note
															// cache
	}
	...
	private void syncLinkedNotebooks() {
		...
		for (int i = 0; i < books.size(); i++) {
			if (errorSharedNotebooksIgnored.containsKey(books.get(i).getGuid()))
				break;
			try {
				logger.log(logger.EXTREME, "Checking notebook: "
						+ books.get(i).getShareName());
				long lastSyncDate = conn.getLinkedNotebookTable()
						.getLastSequenceDate(books.get(i).getGuid());
				int lastSequenceNumber = conn.getLinkedNotebookTable()
						.getLastSequenceNumber(books.get(i).getGuid());

				logger.log(logger.EXTREME, "Last Sequence Number on file: "
						+ lastSequenceNumber);

				// Authenticate to the owner's shard
				String linkedNoteStoreUrl = noteStoreUrlBase
						+ books.get(i).getShardId();
				logger.log(logger.EXTREME, "linkedNoteStoreURL: "
						+ linkedNoteStoreUrl);
				THttpClient linkedNoteStoreTrans = new THttpClient(
						linkedNoteStoreUrl);
				TBinaryProtocol linkedNoteStoreProt = new TBinaryProtocol(
						linkedNoteStoreTrans);
				Client linkedNoteStore = new NoteStore.Client(
						linkedNoteStoreProt, linkedNoteStoreProt);

				linkedAuthResult = null;
				if (books.get(i).getShareKey() != null) {
					logger.log(logger.EXTREME, "Share Key Not Null: "
							+ books.get(i).getShareKey());
					linkedAuthResult = linkedNoteStore
							.authenticateToSharedNotebook(books.get(i)
									.getShareKey(), authToken);
					logger.log(logger.EXTREME, "Authentication Token"
				...
				if (linkedSyncState.getUpdateCount() > lastSequenceNumber) {
					...
					logger.log(logger.EXTREME,
							"Calling syncLinkedNotebook for "
									+ books.get(i).getShareName());
				...
				status.message
						.emit(tr("Error synchronizing \" "
								+ books.get(i).getShareName()
								+ "\". Please verify you still have access to that shared notebook."));
				errorSharedNotebooks.add(books.get(i).getGuid());
				errorSharedNotebooksIgnored.put(books.get(i).getGuid(), books
						.get(i).getGuid());
				logger.log(logger.LOW,
						"Error synchronizing shared notebook.  EDAMNotFound: "
								+ e.getMessage());
				logger.log(logger.LOW, e.getStackTrace());
				error = true;
				e.printStackTrace();
				...
				error = true;
				logger.log(logger.LOW,
						"System error authenticating against shared notebook. "
								+ "Key: " + books.get(i).getShareKey()
								+ " Error:" + e.getMessage());
				e.printStackTrace();
			} catch (TException e) {
				error = true;
	...
	// *************************************************************
	private void syncLinkedNotebook(Client linkedNoteStore,
			LinkedNotebook book, int usn, int highSequence, String token) {
		...
		while (usn < highSequence && !syncError) {
			refreshNeeded = true;
			try {
				SyncChunk chunk = linkedNoteStore.getLinkedNotebookSyncChunk(
						token, book, usn, 10, fullSync);

				// Expunge notes
				syncExpungedNotes(chunk);

				logger.log(logger.EXTREME,
						"Syncing remote notes: " + chunk.getNotesSize());
				syncRemoteNotes(linkedNoteStore, chunk.getNotes(), fullSync,
						linkedAuthResult.getAuthenticationToken());
				logger.log(logger.EXTREME, "Finding new linked tags");
				findNewLinkedTags(linkedNoteStore, chunk.getNotes(),
						linkedAuthResult.getAuthenticationToken());
				// Sync resources
				logger.log(logger.EXTREME,
						"Synchronizing tags: " + chunk.getTagsSize());
				for (int i = 0; i < chunk.getResourcesSize(); i++) {
					syncRemoteResource(linkedNoteStore, chunk.getResources()
							.get(i), linkedAuthResult.getAuthenticationToken());
				}
				logger.log(logger.EXTREME, "Synchronizing linked notebooks: "
						+ chunk.getNotebooksSize());
				syncRemoteLinkedNotebooks(linkedNoteStore,
						chunk.getNotebooks(), false, book);
				syncLinkedTags(chunk.getTags(), book.getGuid());

				// Go through & signal any notes that have changed so we can
				// refresh the user's view
				for (int i = 0; i < chunk.getNotesSize(); i++)
					noteSignal.noteChanged.emit(chunk.getNotes().get(i)
							.getGuid(), null);

				// Expunge Notebook records
				logger.log(logger.EXTREME, "Expunging linked notebooks: "
				...
				for (int i = 0; i < chunk.getExpungedLinkedNotebooksSize(); i++) {
					conn.getLinkedNotebookTable().expungeNotebook(
							chunk.getExpungedLinkedNotebooks().get(i), false);
				}
				usn = chunk.getChunkHighUSN();
				conn.getLinkedNotebookTable().setLastSequenceDate(
						book.getGuid(), chunk.getCurrentTime());
				conn.getLinkedNotebookTable().setLastSequenceNumber(
						book.getGuid(), chunk.getChunkHighUSN());
			} catch (EDAMUserException e) {
				syncError = true;
				status.message
				...
				e.printStackTrace();
				logger.log(logger.LOW,
						tr("EDAM UserException synchronizing linked notbook ")
								+ e.getMessage());
			} catch (EDAMSystemException e) {
				syncError = true;
				status.message
				...
				syncError = true;
				status.message
						.emit(tr("Notebook URL not found. Removing notobook ")
								+ book.getShareName());
				conn.getNotebookTable().deleteLinkedTags(book.getGuid());
				conn.getLinkedNotebookTable().expungeNotebook(book.getGuid(),
						false);
				logger.log(
						logger.LOW,
						tr("Notebook URL not found. Removing notobook ")
								+ e.getMessage());
			} catch (TException e) {
				syncError = true;
				status.message
						.emit(tr("EDAM TException synchronizing linked notbook.  See the log for datails."));
				e.printStackTrace();
				logger.log(
						logger.LOW,
						tr("EDAM TException synchronizing linked notbook.  See the log for datails.")
								+ e.getMessage());
			}
		}
	...
	private void syncLinkedTags(List<Tag> tags, String notebookGuid) {
		...
		if (tags != null) {
			for (int i = 0; i < tags.size() && keepRunning; i++) {
				conn.getTagTable().syncLinkedTag(tags.get(i), notebookGuid,
						false);
			}
		}
		logger.log(logger.EXTREME, "Leaving SyncRunner.syncRemoteTags");
	...
	private void syncRemoteLinkedNotebooks(Client noteStore,
			List<Notebook> notebooks, boolean readOnly, LinkedNotebook linked) {
		logger.log(logger.EXTREME, "Entering SyncRunner.syncRemoteNotebooks");
		if (notebooks != null) {
			for (int i = 0; i < notebooks.size() && keepRunning; i++) {
				try {
					...
					if (!linkedAuthResult.getAuthenticationToken().equals("")) {
						SharedNotebook s = noteStore
								.getSharedNotebookByAuth(linkedAuthResult
										.getAuthenticationToken());
						logger.log(logger.EXTREME,
								"share key:" + s.getShareKey()
										+ " notebookGuid" + s.getNotebookGuid());
						conn.getLinkedNotebookTable().setNotebookGuid(
								s.getShareKey(), s.getNotebookGuid());
					...
					conn.getNotebookTable().syncLinkedNotebook(
							notebooks.get(i), false, readOnly);
				} catch (EDAMUserException e) {
					readOnly = true;
					e.printStackTrace();
				} catch (EDAMNotFoundException e) {
	...
	private void findNewLinkedTags(Client noteStore, List<Note> newNotes,
			String token) {
		if (newNotes == null)
			return;
	...
	private void syncLocalLinkedNoteChanges(Client noteStore,
			LinkedNotebook book) {
		logger.log(logger.EXTREME,
				"Entering SyncRunner.synclocalLinkedNoteChanges");
		String notebookGuid = conn.getLinkedNotebookTable().getNotebookGuid(
				book.getGuid());
		logger.log(logger.EXTREME, "Finding changes for " + book.getShareName()
				+ ":" + book.getGuid() + ":" + notebookGuid);
		...
		for (int i = 0; i < notes.size(); i++) {
			logger.log(logger.EXTREME, "Calling syncLocalNote with key "
					+ linkedAuthResult.getAuthenticationToken());
			syncLocalNote(noteStore, notes.get(i),
					linkedAuthResult.getAuthenticationToken());
		}
		logger.log(logger.EXTREME,
				"Leaving SyncRunner.synclocalLinkedNoteChanges");
	}
