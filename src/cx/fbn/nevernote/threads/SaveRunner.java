...
public class SaveRunner extends QObject implements Runnable {
	...
	public SaveRunner(String logname, String u, String i, String r, String b,
		...
		while (keepRunning) {
			try {
				...
				if (!content.getFirst().equalsIgnoreCase("stop")) {
					...
					// This is a bit of a hack. It causes this thread to pause
					// for 0.2 seconds.
	...
	public void updateNoteContent(String guid, String content) {
		...
		if (fixedContent != null) {
			...
			List<Resource> oldResources = conn.getNoteTable().noteResourceTable
					.getNoteResources(guid, false);
	...
	private void removeObsoleteResources(List<Resource> oldResources,
		...
		if (newResources == null || newResources.size() == 0) {
			for (int i = 0; i < oldResources.size(); i++) {
				conn.getNoteTable().noteResourceTable
						.expungeNoteResource(oldResources.get(i).getGuid());
			}
		}
		for (int i = 0; i < oldResources.size(); i++) {
			boolean matchFound = false;
			for (int j = 0; j < newResources.size(); j++) {
				if (newResources.get(j).equalsIgnoreCase(
						oldResources.get(i).getGuid()))
					matchFound = true;
				if (Global.resourceMap.get(newResources.get(j)) != null) {
					if (Global.resourceMap.get(newResources.get(j))
							.equalsIgnoreCase(oldResources.get(i).getGuid()))
						matchFound = true;
				}
				if (matchFound)
					j = newResources.size();
