...
public class Global {
	...
	public static String[] validVersions = { "1.3", "1.2", "1.1", "1.0",
			"0.99", "0.98", "0.97", "0.96" };
	...
	public static final int trashCounterThreadId = 3; // This should always be
														// the highest thread ID
	public static final int indexThreadId = 4; // Thread for indexing words
	public static final int saveThreadId = 5; // Thread used for processing data
												// to saving content
	...
	public static final int dbThreadId = 9; // This should always be the highest
											// thread ID
	...
	public static boolean disableUploads = false; // Should we disable uploads
													// (used in testing
													// features)
	public static int messageLevel; // The level of messages to write to the log
									// files
	public static String tagDelimeter = ","; // This is used to separate out tag
												// names when entering above
												// note
	public static String attachmentNameDelimeter = "------"; // Used to separate
																// out
																// attachment
																// names in the
																// res directory
	...
	// * Database fields
	public static String databaseName = new String("NeverNote"); // database
																	// name.
																	// used for
																	// multiple
																	// databases
																	...
																	// separate
																	// settings.
	public static String indexDatabaseName = new String("Index"); // searchable
																	// words
																	// database
	public static String resourceDatabaseName = new String("Resources"); // attachments
																			// database
	// ICHANGED
	...
	// When saving a note, this is a list of things we strip out because
	// Evernote hates them
	...
	public static boolean mimicEvernoteInterface; // Try to mimic Evernote or
													// allow multiple notebook
													// selection
	public static HashMap<String, String> resourceMap; // List of attachments
														// for a note.
	public static String cipherPassword = ""; // If the database is encrypted,
	...
	private static FileManager fileManager; // Used to access files &
											// directories

	// Do initial setup
	public static void setup(StartupConfig startupConfig)
		...
		fileManager = new FileManager(startupConfig.getHomeDirPath(),
				startupConfig.getProgramDirPath());
		...
		disableUploads = disableUploads(); // Should we upload anything?
											// Normally true.
		...
		logger = new ApplicationLogger("global.log"); // Setup log for this
														// class
		shortcutKeys = new ShortcutKeys(); // Setup keyboard shortcuts.
		mimicEvernoteInterface = getMimicEvernoteInterface(); // Should we mimic
																// Evernote's
																// notebook
																// behavior
		resourceMap = new HashMap<String, String>(); // Setup resource map used
	...
	public static void saveUserAttributes(UserAttributes attrib) {
		...
		settings.setValue("defaultLocationName",
				attrib.getDefaultLocationName());
		...
		settings.setValue("incomingEmailAddress",
				attrib.getIncomingEmailAddress());
		settings.endGroup();
	}
	...
	public static UserAttributes getUserAttributes() {
		...
		attrib.setDefaultLocationName((String) settings.value(
				"defaultLocationName", ""));
		attrib.setDefaultLatitudeIsSet(false);
		attrib.setDefaultLongitudeIsSet(false);
		attrib.setIncomingEmailAddress((String) settings.value(
	...
	public static long getEvernoteUpdateCount() {
		...
		settings.beginGroup("UserAccounting");
		try {
			String num = (String) settings.value("updateCount",
					new Long(0).toString());
			amt = new Long(num.trim());
		} catch (java.lang.ClassCastException e) {
			amt = 0;
	...
	public static long getUploadLimit() {
		...
		long limit;
		try {
			String num = (String) settings.value("uploadLimit",
					new Long(0).toString());
	...
	// When creating a new note, should it inherit tags that are currently
	// selected?
	public static boolean newNoteWithSelectedTags() {
		settings.beginGroup("General");
		try {
			String text = (String) settings.value("newNoteWithSelectedTags",
					"false");
			settings.endGroup();
			if (text.equalsIgnoreCase("true"))
				return true;
			...
			Boolean value = (Boolean) settings.value("newNoteWithSelectedTags",
					false);
			settings.endGroup();
			return value;
		}
	...
	public static boolean synchronizeDeletedContent() {
		settings.beginGroup("General");
		try {
			String text = (String) settings
					.value("syncDeletedContent", "false");
			settings.endGroup();
			if (text.equalsIgnoreCase("true"))
				return true;
			else
			...
			Boolean value = (Boolean) settings.value("syncDeletedContent",
					false);
			settings.endGroup();
			return value;
		}
	...
	public static boolean isWindowVisible(String window) {
		...
		try {
			...
			if (window.equalsIgnoreCase("noteInformation"))
				defaultValue = false;
			Boolean value = (Boolean) settings.value("showTrayIcon",
					defaultValue);
			settings.endGroup();
			return value;
		}
	...
	public static boolean enableCarriageReturnFix() {
		try {
			settings.beginGroup("Debug");
			String text = (String) settings.value("enableCarriageReturnFix",
					"false");
			settings.endGroup();
			if (text.equalsIgnoreCase("true"))
				return true;
			...
			Boolean value = (Boolean) settings.value("enableCarriageReturnFix",
					false);
			settings.endGroup();
			return value;
		}
	...
	public static boolean enableHtmlEntitiesFix() {
		try {
			settings.beginGroup("Debug");
			String text = (String) settings.value("enableHtmlEntitiesFix",
					"false");
			settings.endGroup();
			if (text.equalsIgnoreCase("true"))
				return true;
			...
			Boolean value = (Boolean) settings.value("enableHtmlEntitiesFix",
					false);
			settings.endGroup();
			return value;
		}
	...
	public static boolean getMimicEvernoteInterface() {
		settings.beginGroup("General");
		try {
			String text = (String) settings.value("mimicEvernoteInterface",
					"true");
			settings.endGroup();
			if (text.equalsIgnoreCase("true"))
				return true;
			...
			Boolean value = (Boolean) settings.value("mimicEvernoteInterface",
					true);
			settings.endGroup();
			return value;
		}
	...
	public static boolean synchronizeOnClose() {
		...
		try {
			String text = (String) settings
					.value("synchronizeOnClose", "false");
			settings.endGroup();
			if (text.equalsIgnoreCase("true"))
				return true;
			else
			...
			Boolean value = (Boolean) settings.value("synchronizeOnClose",
					false);
			settings.endGroup();
			return value;
		}
	...
	public static String getDatabaseUrl() {
		...
		if (val.equals(""))
			val = "jdbc:h2:"
					+ Global.getFileManager().getDbDirPath(Global.databaseName);
		return val;
	}
	...
	public static int getIndexThreadSleepInterval() {
		...
		Integer sleep;
		try {
			String val = (String) settings.value("IndexThreadSleepInterval",
					"300");
	...
	// Add an invalid attribute & element to the database so we don't bother
	// parsing it in the future
	...
	public static boolean getSpellSetting(String value) {
		...
		if (value.equalsIgnoreCase(Configuration.SPELL_IGNOREUPPERCASE))
			return true;
		if (value
				.equalsIgnoreCase(Configuration.SPELL_IGNORESENTENCECAPITALIZATION))
			return true;
		return false;
	}
	...
	// Get/Set how we should display tags (color them, hide unused, or do
	// nothing)
	...
	public static void trace(boolean resetInterval) {
		...
		float interval = (cal.getTimeInMillis() - intervalTraceTime
				.getTimeInMillis());
	...
	public static int calculateThumbnailZoom(String content) {
		...
		if (content.indexOf("application/pdf") == -1) {
			if (content.indexOf("image/") == -1) {
				String text = StringEscapeUtils.unescapeHtml4(content
						.replaceAll("\\<.*?\\>", ""));
				zoom = 2;
				if (text.length() < 500)
					zoom = 2;
	...
	public static String getUpdatesAvailableUrl() {
		settings.beginGroup("Upgrade");
		String text = (String) settings.value("avialableUrl",
				"http://nevernote.sourceforge.net/versions.txt");
		settings.endGroup();
		return text;
	}
	...
	public static String getUpdateAnnounceUrl() {
		settings.beginGroup("Upgrade");
		String text = (String) settings.value("announceUrl",
				"http://nevernote.sourceforge.net/upgrade.html");
		settings.endGroup();
		return text;
	}
	...
	public static boolean indexAttachmentsLocally() {
		settings.beginGroup("Index");
		try {
			String value = (String) settings.value("indexAttachmentsLocally",
					"true");
			settings.endGroup();
			if (value.equals("true"))
				return true;
			...
			Boolean value = (Boolean) settings.value("indexAttachmentsLocally",
					true);
			settings.endGroup();
			return value;
		}
	...
	public static boolean indexImageRecognition() {
		settings.beginGroup("Index");
		try {
			String value = (String) settings.value("indexImageRecognition",
					"true");
			settings.endGroup();
			if (value.equals("true"))
				return true;
			...
			Boolean value = (Boolean) settings.value("indexImageRecognition",
					true);
			settings.endGroup();
			return value;
		}
	...
	// *****************************************************************************
	// Control how tag selection behaves (should they be "and" or "or"
	// selections
	// *****************************************************************************
	public static boolean anyTagSelectionMatch() {
		settings.beginGroup("General");
		try {
			String value = (String) settings.value("anyTagSelectionMatch",
					"false");
			settings.endGroup();
			if (value.equals("true"))
				return true;
			...
			Boolean value = (Boolean) settings.value("anyTagSelectionMatch",
					false);
			settings.endGroup();
			return value;
		}
	...
	// *****************************************************************************
	// Control if a user receives a warning when trying to create a note-to-note
	// link
	...
	public static boolean bypassSynchronizationWarning() {
		settings.beginGroup("User");
		try {
			String value = (String) settings.value(
					"bypassSynchronizationWarning", "false");
			settings.endGroup();
			if (value.equals("true"))
				return true;
			else
			...
			Boolean value = (Boolean) settings.value(
					"bypassSynchronizationWarning", false);
			settings.endGroup();
			return value;
		}
	...
	public static boolean includeTagChildren() {
		settings.beginGroup("General");
		try {
			String value = (String) settings.value("includeTagChildren",
					"false");
			settings.endGroup();
			if (value.equals("true"))
				return true;
			...
			Boolean value = (Boolean) settings.value("includeTagChildren",
					false);
			settings.endGroup();
			return value;
		}
	...
	public static boolean automaticWildcardSearches() {
		...
		try {
			...
			Boolean value = (Boolean) settings
					.value("automaticWildcard", false);
			settings.endGroup();
			return value;
		}
	...
	public static boolean displayRightToLeft() {
		settings.beginGroup("General");
		try {
			String value = (String) settings.value("displayRightToLeft",
					"false");
			settings.endGroup();
			if (value.equals("true"))
				return true;
			...
			Boolean value = (Boolean) settings.value("displayRightToLeft",
					false);
			settings.endGroup();
			return value;
		}
