...
public class ListManager {
	...
	private List<NotebookCounter> notebookCounter; // count of displayed notes
													// in each notebook
	private List<TagCounter> tagCounter; // count of displayed notes for each
											// tag
	...
	public SaveRunner saveRunner; // Thread used to save content. Used because
									// the xml conversion is slowwwww
	QThread saveThread;
	...
	public ListManager(DatabaseConnection d, ApplicationLogger l) {
		...
		notebookCounterRunner = new CounterRunner("notebook_counter.log",
				CounterRunner.NOTEBOOK, Global.getDatabaseUrl(),
		...
		notebookCounterRunner.notebookSignal.countsChanged.connect(this,
				"setNotebookCounter(List)");
		notebookThread = new QThread(notebookCounterRunner,
				"Notebook Counter Thread");
		notebookThread.start();

		tagSignal = new TagSignal();
		// ICHANGED Global.getBehaviorDatabaseUrl()を追加
		tagCounterRunner = new CounterRunner("tag_counter.log",
				CounterRunner.TAG, Global.getDatabaseUrl(),
				Global.getIndexDatabaseUrl(), Global.getResourceDatabaseUrl(),
				Global.getBehaviorDatabaseUrl(), Global.getDatabaseUserid(),
				Global.getDatabaseUserPassword(), Global.cipherPassword);
		tagCounterRunner.setNoteIndex(getNoteIndex());
		tagCounterRunner.tagSignal.countsChanged.connect(this,
				"setTagCounter(List)");
		...
		// ICHANGED Global.getBehaviorDatabaseUrl()を追加
		trashCounterRunner = new CounterRunner("trash_counter.log",
				CounterRunner.TRASH, Global.getDatabaseUrl(),
				Global.getIndexDatabaseUrl(), Global.getResourceDatabaseUrl(),
				Global.getBehaviorDatabaseUrl(), Global.getDatabaseUserid(),
				Global.getDatabaseUserPassword(), Global.cipherPassword);
		trashCounterRunner.trashSignal.countChanged.connect(this,
				"trashSignalReceiver(Integer)");
		...
		// ICHANGED Global.getBehaviorDatabaseUrl()を追加
		saveRunner = new SaveRunner("saveRunner.log", Global.getDatabaseUrl(),
				Global.getIndexDatabaseUrl(), Global.getResourceDatabaseUrl(),
				Global.getBehaviorDatabaseUrl(), Global.getDatabaseUserid(),
				Global.getDatabaseUserPassword(), Global.cipherPassword);
	...
	public void refreshLists(Note n, boolean dirty, String content) {
		if (dirty) {
			// conn.getNoteTable().updateNoteContent(n.getGuid(),
			// n.getContent());
		...
		// get them
		List<cx.fbn.nevernote.sql.NoteTagsRecord> noteTags = conn
				.getNoteTable().noteTagsTable.getAllNoteTags();
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			List<String> tags = new ArrayList<String>();
			List<String> names = new ArrayList<String>();
			for (int j = 0; j < noteTags.size(); j++) {
				if (getMasterNoteIndex().get(i).getGuid()
						.equals(noteTags.get(j).noteGuid)) {
	...
	public void reloadIndexes() {
		...
		enSearch = new EnSearch(conn, logger, "", getTagIndex(),
				Global.getRecognitionWeight());
	...
	public void refreshNoteMetadata() {
		noteModel
				.setNoteMetadata(conn.getNoteTable().getNotesMetaInformation());
	}
	...
	public void setNotebookCounter(List<NotebookCounter> n) {
		notebookCounter = n;
		notebookSignal.refreshNotebookTreeCounts.emit(getNotebookIndex(),
				notebookCounter);
	}
	...
	public void setEnSearch(String t) {
		enSearch = new EnSearch(conn, logger, t, getTagIndex(),
				Global.getRecognitionWeight());
		enSearchChanged = true;
	}
	...
	public void updateNoteContent(String guid, String content) {
		...
		/*
		 * for (int i=0; i<masterNoteIndex.size(); i++) { if
		 * (masterNoteIndex.get(i).getGuid().equals(guid)) {
		 * masterNoteIndex.get(i).setContent(text); i = masterNoteIndex.size();
		 * } } // Update the list tables for (int i=0; i<getNoteIndex().size();
		 * i++) { if (getNoteIndex().get(i).getGuid().equals(guid)) {
		 * getNoteIndex().get(i).setContent(text); i = getNoteIndex().size(); }
		 * }
		 */
		// Check if any new tags were encountered
		/*
		 * if (enml.saveInvalidXML) { List<String> elements =
		 * Global.invalidElements; for (int i=0; i<elements.size(); i++) {
		 * conn.getInvalidXMLTable().addInvalidElement(elements.get(i)); } for
		 * (String key : Global.invalidAttributes.keySet()) { ArrayList<String>
		 * attributes = Global.invalidAttributes.get(key); for (int i=0;
		 * i<attributes.size(); i++) {
		 * conn.getInvalidXMLTable().addInvalidAttribute(key,
		 * attributes.get(i)); } } }
		 */
	...
	public void updateNoteGeoTag(String guid, Double lon, Double lat, Double alt) {
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				...
				getMasterNoteIndex().get(i).getAttributes()
	...
	public void updateNoteSequence(String guid, int sequence) {
		...
		for (int i = 0; i < noteModel.getMasterNoteIndex().size(); i++) {
			if (noteModel.getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				noteModel.getMasterNoteIndex().get(i)
						.setUpdateSequenceNum(sequence);
	...
	public List<Tag> findAllChildrenRecursive(String guid, List<Tag> tags) {
		...
		// Start looping through the tags. If we find a tag which has a parent
		// that
		// matches guid, then we add it to the list of tags & search for its
		// children.
		for (int i = 0; i < getTagIndex().size(); i++) {
			if (getTagIndex().get(i).getParentGuid() != null
					&& getTagIndex().get(i).getParentGuid().equals(guid)) {
				tags.add(getTagIndex().get(i));
				tags = findAllChildrenRecursive(getTagIndex().get(i).getGuid(),
						tags);
			}
		}
		return tags;
	...
	public void renameStack(String oldName, String newName) {
		for (int i = 0; i < getNotebookIndex().size(); i++) {
			if (getNotebookIndex().get(i).getStack() != null
					&& getNotebookIndex().get(i).getStack()
	...
	// Check if a note matches the currently selected notebooks, tags, or
	// attribute searches.
	public boolean filterRecord(Note n) {
		...
		if (goodTag) {
			...
			if (!Global.containsFilter.hasSelection())
				...
				goodContains = Global.containsFilter.check(conn.getNoteTable(),
						n);

			if (goodCreatedSince && goodCreatedBefore && goodChangedSince
					&& goodChangedBefore && goodContains)
				return true;
		}
		return false;
	}
	...
	public void countNotebookResults(List<Note> index) {
		logger.log(logger.EXTREME,
				"Entering ListManager.countNotebookResults()");
		notebookCounterRunner.abortCount = true;
	...
	private boolean filterByTag(List<String> noteTags) {
		...
		if (!Global.anyTagSelectionMatch()) {
			...
			for (int i = 0; i < selectedTags.size(); i++) {
				...
				if (noteTags.contains(selectedGuid))
					return true;
				// If we have a match with one of the children tags && we should
				// include child tags
				if (Global.includeTagChildren()
						&& checkNoteForChildTags(selectedGuid, noteTags))
					return true;
			}
			return false;
		}
	...
	public void compactDatabase() {
		...
		// IndexConnection idx = new IndexConnection(logger,
		// "nevernote-compact");
		// idx.dbSetup();
		// idx.dbShutdown();
	}
	...
	public List<String> scanNoteForResources(Note n) {
		...
		if (!result.success) {
			logger.log(logger.MEDIUM,
					"Parse error when scanning note for resources.");
	...
	public void reloadNoteTagNames(String tagGuid, String newName) {
		...
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			for (int j = 0; j < getMasterNoteIndex().get(i).getTagGuids()
					.size(); j++) {
				if (getMasterNoteIndex().get(i).getTagGuids().get(j)
						.equals(tagGuid)) {
