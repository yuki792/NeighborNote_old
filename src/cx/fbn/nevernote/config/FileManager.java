...
public class FileManager {
	...
	private static final Pattern ALL_PATH_SEPARATORS_REGEX = Pattern
			.compile("[/\\\\]");
	 ...
	 * 
	 * @param homeDirPath
	 *            the installation dir containing db/log/res directories, must
	 *            exist
	 * @throws InitializationException
	 *             for missing directories or file permissions problems
	 */
	public FileManager(String homeDirPath, String programDirPath)
			throws InitializationException {
		if (homeDirPath == null) {
		...
		if (programDirPath == null) {
			throw new IllegalArgumentException(
					"programDirPath must not be null");
		}
	...
	/**
	 * Get a path below the base user home directory, using native
	 * {@link File#separator}. This will contain backslashes on Windows.
	 */
	...
	/**
	 * Get a path below the 'spell' directory, using native
	 * {@link File#separator}. This will contain backslashes on Windows.
	 */
	...
	/**
	 * Get a path below the 'images' directory, using native
	 * {@link File#separator}. This will contain backslashes on Windows.
	 */
	...
	/**
	 * Get a path to the 'res' directory, terminated with native
	 * {@link File#separator}. This will contain backslashes on Windows.
	 */
	...
	public String getResDirPathSpecialChar(String relativePath) {
		return resDirPath
				+ toPlatformPathSeparator(relativePath).replace("#", "%23");
	}
	...
	/**
	 * Get a path below the 'translate' directory, using native
	 * {@link File#separator}. This will contain backslashes on Windows.
	 */
	...
	private static String toPlatformPathSeparator(String relativePath) {
		// Sometimes a space in the file name comes across as a %20. This is to
		// put it back as a space.
		...
		// because they have special meaning in the replacement string of
		// Matcher.replaceAll
	 ...
	 * 
	 * @throws InitializationException
	 *             for file deletion failures
	 */
	private static void deleteTopLevelFiles(File dir, boolean exitOnFail)
			throws InitializationException {
		...
		for (File f : toDelete) {
			if (!f.delete() && exitOnFail) {
				throw new InitializationException("Failed to delete file: '"
						+ f + "'");
			}
		}
	}
	...
	/**
	 * @throws InitializationException
	 *             for bad file permissions, or a file instead of a directory
	 */
	private static void createDirOrCheckWriteable(File dir)
			throws InitializationException {
		if (dir.isDirectory()) {
			// Dir exists, check permissions
			if (!dir.canRead()) {
				throw new InitializationException("Directory '" + dir
			...
			throw new InitializationException("Expected directory '" + dir
					+ "' but found a file instead");
		}
	}

	/**
	 * @throws InitializationException
	 *             if non-existent, bad file permissions, or a file instead of a
	 *             directory
	 */
	private static void checkExistingReadableDir(File dir)
			throws InitializationException {
		if (dir.isDirectory()) {
			// Dir exists, check permissions
			if (!dir.canRead()) {
	...
	private static void checkExistingWriteableDir(File dir)
			throws InitializationException {
	 ...
	 */
	public void purgeResDirectory(boolean exitOnFail)
			throws InitializationException {
