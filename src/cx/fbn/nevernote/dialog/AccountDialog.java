...
public class AccountDialog extends QDialog {
	...
	public AccountDialog() {
		...
		if (uploadAmt > 0)
			textGrid.addWidget(new QLabel(uploadAmt.toString() + unit + " ("
					+ pct + "%)"), 4, 2);
		else
		...
		// attribGrid.addWidget(createIncomingEmailField(attrib.getIncomingEmailAddress()+"@"+server),
		// 1,2);
	...
	/*
	 * private QWidget createIncomingEmailField(String email){ QTextEdit
	 * emailTextEdit = new QTextEdit();
	 * 
	 * String emailLinkFormat="<a href=\"mailto:%1$s\">%1$s</a>"; String
	 * emailHtml = String.format(emailLinkFormat, email);
	 * 
	 * emailTextEdit.setLineWrapMode(LineWrapMode.NoWrap);
	 * 
	 * QTextDocument doc = new QTextDocument(); doc.setHtml(emailHtml);
	 * doc.setMaximumBlockCount(1);
	 * 
	 * emailTextEdit.setDocument(doc); emailTextEdit.setReadOnly(true);
	 * 
	 * //set background color as for disabled control QPalette palette = new
	 * QPalette(); QColor backgroundColor =
	 * QApplication.palette().color(ColorRole.Window);
	 * palette.setColor(ColorRole.Base, backgroundColor);
	 * 
	 * emailTextEdit.setAutoFillBackground(true);
	 * emailTextEdit.setPalette(palette);
	 * 
	 * //remove frame borders emailTextEdit.setFrameShape(QFrame.Shape.NoFrame);
	 * 
	 * //set height of emailTextEdit actually single line QFontMetrics
	 * fontMetrics = emailTextEdit.fontMetrics(); //we also add some pixels to
	 * avoid showing scrollbars int height = fontMetrics.height() +
	 * emailTextEdit.frameWidth()*2 + 8; emailTextEdit.setFixedHeight(height);
	 * 
