...
public class IgnoreSync extends QDialog {
	...
	public IgnoreSync(List<Notebook> allBooks, List<Notebook> archive,
			List<Tag> allTags, List<Tag> ignoreTags,
			List<LinkedNotebook> allLinkedNotebooks,
			List<LinkedNotebook> ignoreLinkedNotebooks) {
		...
		ignoreBookList.itemSelectionChanged.connect(this,
				"ignoreBookSelected()");
		...
		syncLinkedNotebookList.itemSelectionChanged.connect(this,
				"syncLinkedNotebookSelected()");
		ignoreLinkedNotebookList.itemSelectionChanged.connect(this,
				"ignoreLinkedNotebookSelected()");
		leftLinkedNotebookButton.clicked.connect(this,
				"toOpenLinkedNotebookList()");
		rightLinkedNotebookButton.clicked.connect(this,
				"toClosedLinkedNotebookList()");
		...
		for (int i = 0; i < allTags.size(); i++) {
			...
			for (int j = 0; j < ignoreTags.size(); j++) {
				if (ignoreTags.get(j).getName()
						.equalsIgnoreCase(allTags.get(i).getName())) {
					found = true;
					j = ignoreTags.size();
				}
			}
			if (!found) {
				QListWidgetItem item = new QListWidgetItem(allTags.get(i)
						.getName());
				item.setSelected(false);
		...
		for (int i = 0; i < ignoreTags.size(); i++) {
			QListWidgetItem item = new QListWidgetItem(ignoreTags.get(i)
					.getName());
			item.setSelected(false);
		...
		for (int i = 0; i < allLinkedNotebooks.size(); i++) {
			...
			for (int j = 0; j < ignoreLinkedNotebooks.size(); j++) {
				if (ignoreLinkedNotebooks
						.get(j)
						.getShareName()
						.equalsIgnoreCase(
								allLinkedNotebooks.get(i).getShareName())) {
					found = true;
					j = ignoreLinkedNotebooks.size();
				}
			}
			if (!found) {
				QListWidgetItem item = new QListWidgetItem(allLinkedNotebooks
						.get(i).getShareName());
				item.setSelected(false);
		...
		for (int i = 0; i < ignoreLinkedNotebooks.size(); i++) {
			QListWidgetItem item = new QListWidgetItem(ignoreLinkedNotebooks
					.get(i).getShareName());
			item.setSelected(false);
