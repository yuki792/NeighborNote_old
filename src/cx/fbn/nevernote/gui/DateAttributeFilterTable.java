...
public class DateAttributeFilterTable {
	...
	public DateAttributeFilterTable(boolean since, boolean created) {
		...
		for (DateAttributeFilterFactory.FilterType type : DateAttributeFilterFactory.FilterType
				.values()) {
			table.add(DateAttributeFilterFactory
					.getFilter(type, since, created));
		}
	}
	...
	public boolean check(Note n) {
		for (int i = 0; i < table.size(); i++) {
			if (table.get(i).isSet() && !table.get(i).attributeCheck(n))
				return false;
		}
		return true;
	}
