...
public class Thumbnailer extends QObject {
	...
	public Thumbnailer(ApplicationLogger logger, DatabaseConnection conn,
			ListManager l, ThumbnailRunner r) {
		mutex = new QMutex();
		zoom = 1;
	...
	public void loadContent(String guid, QByteArray html, int zoom) {
		this.zoom = zoom;
		this.guid = guid;
		page.mainFrame().setScrollBarPolicy(Orientation.Horizontal,
				ScrollBarPolicy.ScrollBarAlwaysOff);
		page.mainFrame().setScrollBarPolicy(Orientation.Vertical,
				ScrollBarPolicy.ScrollBarAlwaysOff);
	...
	private String loadFinished(Boolean ok) {
		...
		if (image.isNull()) {
			logger.log(logger.EXTREME,
					"Error rendering thumbnail image.  Aborting");
			mutex.unlock();
			return null;
		}
		logger.log(logger.EXTREME, "Saving image isNull=" + image.isNull()
				+ " Size=" + image.size());
