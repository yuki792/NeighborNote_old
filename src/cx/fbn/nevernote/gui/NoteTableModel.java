...
public class NoteTableModel extends QAbstractTableModel {
	...
	public Object data(QModelIndex index, int role) {
		...
		case Qt.ItemDataRole.BackgroundRole: {
			String guid = (String) valueAt(index.row(),
	...
	@Override
	public java.lang.Object headerData(int section, Qt.Orientation orientation,
			int role) {
	...
	@Override
	public boolean setHeaderData(int section, Qt.Orientation orientation,
	...
	public void updateNoteTitle(String guid, String title) {
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				...
				QModelIndex idx = createIndex(i, Global.noteTableTitlePosition,
						nativePointer());
	...
	// Update a note title
	public void updateNoteTags(String guid, List<String> tags,
			List<String> names) {
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				...
				QModelIndex idx = createIndex(i, Global.noteTableTagPosition,
						nativePointer());
	...
	public void updateNoteCreatedDate(String guid, QDateTime date) {
		...
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				getMasterNoteIndex().get(i).setCreated(
						(long) date.toTime_t() * 1000);
				QModelIndex idx = createIndex(i,
						Global.noteTableCreationPosition, nativePointer());
				setData(idx,
						new Long(getMasterNoteIndex().get(i).getCreated()),
	...
	public void updateNoteSubjectDate(String guid, QDateTime date) {
		...
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				getMasterNoteIndex().get(i).getAttributes()
						.setSubjectDate((long) date.toTime_t() * 1000);
				QModelIndex idx = createIndex(i,
						Global.noteTableSubjectDatePosition, nativePointer());
				setData(idx, new Long(getMasterNoteIndex().get(i)
						.getAttributes().getSubjectDate()),
						Qt.ItemDataRole.EditRole);
				i = getMasterNoteIndex().size();
			}
		}
		...
		for (int i = 0; i < getNoteIndex().size(); i++) {
			if (getNoteIndex().get(i).getGuid().equals(guid)) {
				getNoteIndex().get(i).getAttributes()
						.setSubjectDate((long) date.toTime_t() * 1000);
				i = getNoteIndex().size();
			}
		}
	...
	public void updateNoteChangedDate(String guid, QDateTime date) {
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				getMasterNoteIndex().get(i).setUpdated(
						(long) date.toTime_t() * 1000);
				QModelIndex idx = createIndex(i,
						Global.noteTableChangedPosition, nativePointer());
				setData(idx, new Long(getMasterNoteIndex().get(i)
						.getAttributes().getSubjectDate()),
						Qt.ItemDataRole.EditRole);
				i = getMasterNoteIndex().size();
	...
	public void updateNoteGuid(String oldGuid, String newGuid) {
		...
		for (int i = 0; i < getNoteIndex().size(); i++) {
			if (getNoteIndex().get(i).getGuid() != null
					&& getNoteIndex().get(i).getGuid().equals(oldGuid)) {
		...
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid() != null
					&& getMasterNoteIndex().get(i).getGuid().equals(oldGuid)) {
				getMasterNoteIndex().get(i).setGuid(newGuid);
				// QModelIndex idx = createIndex(i,
				// Global.noteTableGuidPosition, nativePointer());
				// setData(idx, new
				// String(getMasterNoteIndex().get(i).getGuid()),
				// Qt.ItemDataRole.EditRole);
		...
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				...
				for (int j = 0; j < listManager.getNotebookIndex().size(); j++) {
					if (listManager.getNotebookIndex().get(j).getGuid()
							.equals(notebookGuid)) {
						String name = listManager.getNotebookIndex().get(j)
								.getName();
						QModelIndex idx = createIndex(i,
	...
	public void updateNoteAuthor(String guid, String author) {
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				...
				QModelIndex idx = createIndex(i,
						Global.noteTableAuthorPosition, nativePointer());
	...
	public void updateNoteSourceUrl(String guid, String url) {
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				...
				getMasterNoteIndex().get(i).getAttributes()
						.setSourceURLIsSet(true);
				QModelIndex idx = createIndex(i,
						Global.noteTableSourceUrlPosition, nativePointer());
	...
	public void updateNoteSyncStatus(String guid, boolean sync) {
		...
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				QModelIndex idx = createIndex(i,
						Global.noteTableSynchronizedPosition, nativePointer());
				String value;
				if (!sync)
					value = tr("false");
				else
	...
	public void updateNotePinnedStatus(String guid, boolean pinned) {
		...
		for (int i = 0; i < getMasterNoteIndex().size(); i++) {
			if (getMasterNoteIndex().get(i).getGuid().equals(guid)) {
				QModelIndex idx = createIndex(i,
						Global.noteTablePinnedPosition, nativePointer());
				String value;
				if (metaData.containsKey(guid))
					;
	...
	public void updateNoteTitleColor(String guid, int color) {
		...
		if (metaData.containsKey(guid)
				&& metaData.get(guid).getColor() != color) {
