...
public class TagTreeWidget extends QTreeWidget {
	...
	public void updateCounts(List<TagCounter> counts) {
		...
		for (int i = 0; i < children.size(); i++) {
			...
			if (!showAllTags
					&& (Global.tagBehavior().equalsIgnoreCase(
							"HideInactiveCount") || Global.tagBehavior()
							.equalsIgnoreCase("NoHideInactiveCount")))
		...
		for (int i = 0; i < counts.size(); i++) {
			for (int j = 0; j < children.size(); j++) {
				...
				if (counts.get(i).getGuid().equals(guid)) {
					children.get(j).setText(1,
							new Integer(counts.get(i).getCount()).toString());
	...
	public void dragEnterEvent(QDragEnterEvent event) {
		...
		if (event.source() == this) {
			...
			event.mimeData().setData("application/x-nevernote-tag",
	...
	public boolean dropMimeData(QTreeWidgetItem parent, int index,
		...
		if (data.hasFormat("application/x-nevernote-tag")) {
			...
			// Check we don't do a dumb thing like move a parent to a child of
			// itself
		...
		if (data.hasFormat("application/x-nevernote-note")) {
			String notebookGuid = db.getTagTable().getNotebookGuid(
					parent.text(2));
			...
			for (String element : noteGuidArray) {
				...
				// 2.) Check that that tag is valid for that notebook or the tag
				// isn't notebook specific
				// 3.) Check that the notebook isn't read only.
				if (!db.getNoteTable().noteTagsTable.checkNoteNoteTags(
						element.trim(), parent.text(2))
						&& (notebookGuid == null
								|| n.getNotebookGuid().equalsIgnoreCase(
										notebookGuid) || notebookGuid
									.equals(""))
						&& !db.getNotebookTable().isReadOnly(
								n.getNotebookGuid())) {
					db.getNoteTable().noteTagsTable.saveNoteTag(element.trim(),
							parent.text(2));
	...
	private void itemClicked() {
		...
		if (selectedItem.size() == 1) {
			if (selectedItem.get(0).text(0).equalsIgnoreCase(selectedTag)
					&& !rightButtonClicked) {
				selectedTag = "";
				clearSelection();
			} else {
