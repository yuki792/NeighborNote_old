...
public class NotebookTreeWidget extends QTreeWidget {
	...
	public QIcon findDefaultIcon(String guid, String name,
			List<String> localBooks, boolean isPublished) {
		...
		if (localBooks.contains(guid)
				&& (name.equalsIgnoreCase("Conflicting Changes") || name
	...
	public void load(List<Notebook> books, List<String> localBooks) {
		...
		for (int i = 0; i < books.size(); i++) {
			...
			if (icons != null && !icons.containsKey(book.getGuid())) {
				QIcon icon = findDefaultIcon(book.getGuid(), book.getName(),
						localBooks, book.isPublished());
		...
		if (Global.mimicEvernoteInterface) {
			String iconPath = new String("classpath:cx/fbn/nevernote/icons/");
			QIcon allIcon = db.getSystemIconTable().getIcon("All Notebooks",
					"ALLNOTEBOOK");
		...
		for (int i = 0; i < root.childCount(); i++) {
			for (int j = 0; j < expandedStacks.size(); j++) {
				if (root.child(i).text(0)
						.equalsIgnoreCase(expandedStacks.get(j))) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (child != null && child.childCount() > 0) {
				...
				for (int j = 0; j < count; j++) {
					...
					int childCount = updateCounts(child, books, counts, blue,
							black);
	...
	private int updateCounts(QTreeWidgetItem child, List<Notebook> books,
			List<NotebookCounter> counts, QBrush blue, QBrush black) {
		int total = 0;
		if (child != null) {
			String guid = child.text(2);
			child.setText(1, "0");
			...
			for (int j = 0; j < counts.size(); j++) {
				if (counts.get(j).getGuid().equals(guid)) {
					child.setText(1,
							new Integer(counts.get(j).getCount()).toString());
	...
	// Return a list of the notebook guids, ordered by the current display
	// order.
	...
	public void dragEnterEvent(QDragEnterEvent event) {
		...
		if (event.source() == this) {
			event.mimeData().setData("application/x-nevernote-notebook",
					new QByteArray(currentItem().text(2)));
	...
	protected void dragMoveEvent(QDragMoveEvent event) {
		...
		if (treeItem != null) {
			/*
			 * if
			 * (!previousMouseOver.text(0).equalsIgnoreCase(treeItem.text(0))) {
			 * previousMouseOver.setSelected(previousMouseOverWasSelected);
			 * previousMouseOverWasSelected = treeItem.isSelected();
			 * previousMouseOver = treeItem; blockSignals(true);
			 * treeItem.setSelected(true); blockSignals(false); }
			 */
		}
		if (event.mimeData().hasFormat("application/x-nevernote-note")) {
	...
	@Override
	public boolean dropMimeData(QTreeWidgetItem parent, int index,
			QMimeData data, Qt.DropAction action) {
		if (data.hasFormat("application/x-nevernote-notebook")) {
			return false;
		}

		// This is really dead code. it is the beginning of logic to create
		// stacks by
		// dragging.
		if (data.hasFormat("application/x-nevernote-notebook")) {
			...
			if (!parent.text(2).equalsIgnoreCase("STACK")) {
				...
				if (parent.parent() != null) {
					...
					Qt.Alignment ra = new Qt.Alignment(
		...
		if (data.hasFormat("application/x-nevernote-note")) {
			...
			for (String element : noteGuidArray) {
				Note n = db.getNoteTable().getNote(element.trim(), false,
				...
				if (!parent.text(2).equalsIgnoreCase("")
						...
						&& !(n.getNotebookGuid().equalsIgnoreCase(parent
								.text(2)))) {
					noteSignal.notebookChanged.emit(element.trim(),
							parent.text(2));
					if (db.getNotebookTable().isLinked(parent.text(2))) {
						noteSignal.tagsChanged.emit(element.trim(),
